...
def main(args):
    ...
    if args.template:
        if args.template.endswith('nii') or args.template.endswith('nii.gz'):
            template_file_list = [args.template]
        else:
            template_file_list = glob.glob(f'{args.template}/*.nii*', recursive=True)
        if args.template_name:
            template_file_list = list(filter(lambda x: args.template_name in x, template_file_list))
        template_resample_file_list = list(map(lambda x: replace_suffix(x, f'_resample.nii.gz'), template_file_list))
        template_synthseg_file_list = list(map(lambda x: replace_suffix(x, f'_synthseg.nii.gz'), template_file_list))
        template_synthseg33_file_list = list(
            map(lambda x: replace_suffix(x, f'_synthseg33.nii.gz'), template_file_list))
    else:
    ...
    if args.template:
        if args.cmb:
            cmb_file_list = list(map(lambda x: replace_suffix(x, f'_{args.cmb_file}.nii.gz'), template_file_list))
        else:
            cmb_file_list = []
        if args.dwi:
            dwi_file_list = list(map(lambda x: replace_suffix(x, f'_{args.dwi_file}.nii.gz'), template_file_list))
        else:
            dwi_file_list = []
        if args.wmh:
            wmh_file_list = list(map(lambda x: replace_suffix(x, f'_{args.wmh_file}.nii.gz'), template_file_list))
        else:
            wmh_file_list = []
        ...
        wm_file_list = list(map(lambda x: replace_suffix(x, f'_david.nii.gz'), template_file_list))
    ...
    for i in range(len(file_list)):
        try:
            if args.template:
                ...
                synth_seg.run_segmentations33(path_images=resample_file_list[i],
                                              path_segmentations33=synthseg33_file_list[i], )
                resample_one(template_file_list[i], template_resample_file_list[i])
                synth_seg.run(path_images=template_resample_file_list[i],
                              path_segmentations=template_synthseg_file_list[i],
                              path_segmentations33=template_synthseg33_file_list[i],
                              )

                synthseg_nii = nib.load(template_synthseg_file_list[i])
                synthseg_array = synthseg_nii.get_fdata()
                synthseg33_nii = nib.load(template_synthseg33_file_list[i])
                ...
                if args.cmb:
                    cmb_basename = cmb_file_list[i].replace('.nii.gz', '')
                    ...
                    print(f'cmb {flirt_cmd_str}')
                if args.dwi:
                    dwi_basename = dwi_file_list[i].replace('.nii.gz', '')
