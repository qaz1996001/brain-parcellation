...
if __name__ == '__main__':
    ...
    root_dir_path = pathlib.Path(r'D:\00_Chen\Task07_CT_DWI\data\clean_data')
    file_list = list(root_dir_path.rglob('*CT_descalp_synthseg33.nii.gz'))
    # output_file_list = list(map(lambda x: root_dir_path.joinpath(x.name.replace('.nii.gz','original.nii.gz'))
    #                             ,file_list))
    for i in range(len(file_list)):
        resample_file_path = file_list[i]
        output_file_path = resample_file_path.parent.joinpath(resample_file_path.name.replace('.nii.gz','_original.nii.gz'))
        original_file_path = list(resample_file_path.parent.glob('*CT_descalp.nii.gz*'))[0]
        # print(resample_file_path)
        # print(output_file_path)
        # print(original_file_path)
        resample_to_original(str(resample_file_path), str(original_file_path), str(output_file_path))
        # break
    #
