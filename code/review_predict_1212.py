...
class SynthSeg:
    ...
    def build_unet_5_class_model(self, path_model_segmentation,
                                 labels_segmentation,
                                 labels_denoiser, ):
        assert os.path.isfile(path_model_segmentation), "The provided model path does not exist."
        ...
        net_unet2.load_weights(path_model_segmentation, by_name=True)
        net_robust_5c = Model(inputs=net_unet2.inputs, outputs=net_unet2.get_layer('lambda_4').output)

        return net_robust_5c
    ...
    def run_segmentations5(self, path_images, path_segmentations5):
        ...
        if self.net_unet_5_class is None:
            net_unet_5_class = self.build_unet_5_class_model(
                path_model_segmentation=path_model_segmentation,
                labels_segmentation=labels_segmentation,
                labels_denoiser=labels_denoiser, )
            self.net_unet_5_class = net_unet_5_class
        else:
            net_unet_5_class = self.net_unet_5_class
        ...
        unet2_output = net_unet_5_class.predict(image)
        seg, posteriors, volumes = self.postprocess(post_patch_seg=unet2_output,
                                                    post_patch_parc=None,
                                                    shape=shape,
                                                    pad_idx=pad_idx,
                                                    ...
                                                    v1=True)
        utils.save_volume(seg, aff, h, path_segmentations5, dtype='int32')
...
if __name__ == '__main__':
    ...
    # np_data1 = np.load(os.path.join(synth_seg.LABELS_DIR,'synthseg_denoiser_labels_2.0.npy'))
    # # np_data1 = np.load(os.path.join(synth_seg.LABELS_DIR, 'synthseg_denoiser_labels_2.0.npy'))
    # np_data2 = np.load(os.path.join(synth_seg.LABELS_DIR, 'synthseg_segmentation_labels_2.0.npy'))
    # print(np_data1.shape)
    # print(np.unique(np_data1))
    # print(np_data2.shape)
    # print(np.unique(np_data2))

    data_root_path = pathlib.Path(r'D:\00_Chen\Task04_\SynthSeg_predict')
    file_list = list(data_root_path.rglob('*'))
    resample_file_list = list(map(lambda x: x.parent.joinpath(replace_suffix(x.name, f'_resample.nii.gz')),
                                  file_list))
    synthseg_file_list = list(map(lambda x: x.parent.joinpath(replace_suffix(x.name, f'_synthseg5.nii.gz')),
                                  file_list))
    for i in range(len(file_list)):
        ...
        synth_seg.run_segmentations5(path_images=str(resample_file_list[i]),
                                     path_segmentations5=str(synthseg_file_list[i]), )
