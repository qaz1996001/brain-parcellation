...
class WhiteMatterParcellation:
    """
        Class for white matter parcellation and mapping SYNSEG labels to FreeSurfer and David labels.

        Attributes
        ----------
        synseg_label_freesurfer_GM_mapping : dict
            Mapping from SYNSEG labels to FreeSurfer labels for gray matter in left and right hemisphere.
        white_matter_mapping : dict
            Mapping from SYNSEG labels to white matter labels in left and right hemisphere.
        cerebral_white_matter : dict
            Cerebral white matter labels for left and right hemisphere.
        synseg_label_david_mapping : dict
            Mapping from SYNSEG labels to David labels for gray matter in left and right hemisphere.
        white_matter_david_mapping : dict
            Mapping from white matter labels to David labels in left and right hemisphere.
        gray_matter_david_mapping : dict
            Mapping from gray matter labels to David labels in left and right hemisphere.
        decimal_places : int
            Decimal places for calculations.
        scaling_factor : int
            Scaling factor for decimal places.
        re_white_matter_mapping : dict
            Reverse mapping for white matter labels.

        Methods
        -------
        label_to_david_label(label_array)
            Maps SYNSEG labels to FreeSurfer and David labels.
        synseg_label_to_freesurfer_GM(label_array)
            Maps SYNSEG labels to FreeSurfer labels for gray matter.
        white_matter_parcellation(label_array)
            Performs white matter parcellation based on SYNSEG labels.
        re_run(synthseg_array_wm, synthseg_array_cc, synthseg_array_ec)
            Re-runs white matter parcellation based on additional arrays.
        re_white_matter_parcellation(label_array)
            Re-runs white matter parcellation based on SYNSEG labels.
        run(synthseg_array)
            Runs the entire white matter parcellation process.
    ...
    white_matter_mapping = {
        # left hemisphere white matter labels
        'left_hemi': {1001: 3001,
                      1003: 3003,
                      1004: 3004,
                      1005: 3005,
        ...
        # right hemisphere white matter labels
    ...
    gray_matter_david_mapping = {
        # left hemisphere gray matter labels
        'left_hemi': {1001: 112,
                      1006: 113,
                      1004: 114,
                      1005: 115,
        ...
        # right hemisphere gray matter labels
        'right_hemi': {2001: 212,
                       2006: 213,
                       2004: 214,
                       2005: 215,
    ...
    def label_to_david_label(cls, label_array):
        ...
        label_array : numpy.ndarray
            Array containing SYNSEG labels.

        Returns
        -------
        numpy.ndarray
            Array with mapped FreeSurfer and David labels.
    ...
    def synseg_label_to_freesurfer_GM(cls, label_array):
        ...
        -------
        numpy.ndarray
            Array with mapped FreeSurfer gray matter labels.
    ...
    def white_matter_parcellation(cls, label_array):
        ...
        -------
        numpy.ndarray
            Array with mapped white matter labels.
    ...
    def re_run(cls, synthseg_array_wm, synthseg_array_cc, synthseg_array_ec):
        ...
        Parameters
        ----------
        synthseg_array_wm : numpy.ndarray
            Array with original white matter labels.
        synthseg_array_cc : numpy.ndarray
            Array with corpus callosum labels.
        synthseg_array_ec : numpy.ndarray
            Array with external capsule and internal capsule labels.

        Returns
        -------
        numpy.ndarray
            Array with re-run white matter parcellation.

        """
        out_array = synthseg_array_wm.copy()
        # Iterate through Corpus Callosum white matter regions
        for k in CorpusCallosumParcellation.cc_white_matter:
            # Get the target labels for Corpus Callosum
            cc_target = CorpusCallosumParcellation.cc_target[k]
            # Create a mask for the current Corpus Callosum white matter region
            base_mask = (out_array == CorpusCallosumParcellation.cc_white_matter[k])
            # Create a mask for the target labels in the corresponding SYNSEG array
            target_mask = np.isin(synthseg_array_cc, cc_target)
            # Identify the difference between the base mask and the target mask
            diff_mask = np.logical_and(base_mask,
                                       np.logical_not(target_mask))
            # Update the output array by assigning cerebral white matter labels to the differing elements
            out_array[diff_mask] = WhiteMatterParcellation.cerebral_white_matter[k]
        # Iterate through ECIC white matter regions
        for k in ECICParcellation.ec_ic_white_matter:
            # Get the target labels for ECIC
            ec_ic_target_list = list(set(ECICParcellation.ec_ic_parcellation_mapping[k].values()))
            # Create a mask for the current ECIC white matter region
    ...
    def re_white_matter_parcellation(cls, label_array):
        ...
        # 輸出 nii.gz 的新 array
        new_label_array = label_array.copy()
        # Iterate through cerebral white matter hemispheres (left and right)
        for k in cls.cerebral_white_matter:
            # 2 or 41
            # Identify indices of SYNSEG labels corresponding to cerebral white matter (2 or 41)
            index = np.argwhere(label_array == cls.cerebral_white_matter[k])
            # Get the keys for reclassification white matter mapping
            white_matter_mapping_keys = list(cls.re_white_matter_mapping[k].keys())
            # Initialize an array to store loss values
            np_loss = np.zeros((index.shape[0], len(white_matter_mapping_keys)))
            np_loss[:, :] = 999999
            # Iterate through potential white matter labels (3001 ... 4007)
            for i in range(len(white_matter_mapping_keys)):
                # 3001 ... 4007
                # Identify indices of SYNSEG labels corresponding to the current white matter label
                label_index = np.argwhere(label_array == white_matter_mapping_keys[i])
                loss_list = []
                index_sub_arg_list = []
                # Z 軸切片
                # Iterate through unique Z-axis slices
                for j in np.unique(label_index[:, 2]):
                    # Extract indices and label indices for the current Z-axis slice
                    index_sub = index[index[:, 2] == j]
                    index_sub_arg = np.argwhere(index[:, 2] == j)
                    label_index_sub = label_index[label_index[:, 2] == j]
                    # Check if both sets of indices are non-empty
                    if (index_sub.shape[0] > 0) and (label_index_sub.shape[0] > 0):
                        # Calculate and store loss values for each slice
                        loss_min = loss_distance(index_sub, label_index_sub, cls.decimal_places)
                        loss_list.append(loss_min.numpy())
                        index_sub_arg_list.append(index_sub_arg)
                    else:
                        continue
                # If there are non-empty index and label index sets
                if len(index_sub_arg_list) > 0:
                    # Concatenate index arguments and update loss array
                    index_sub_arg = np.concatenate(index_sub_arg_list)
                    np_loss[index_sub_arg, i] = np.concatenate(loss_list).reshape(-1, 1)
                else:
                    continue
            # Identify the index of the minimum loss for each element in the original white matter indices
            new_label = np_loss.argmin(axis=1)
            # 指定分類
            # Assign the mapped white matter labels based on the minimum loss
            for i in np.unique(new_label):
                select_index = index[np.argwhere(new_label == i)].squeeze()
                # Check if the index is 2D or 3D
                if select_index.ndim == 2:
                    ...
                    new_label_array[select_index[0], select_index[1], select_index[2]] = \
                        cls.re_white_matter_mapping[k][white_matter_mapping_keys[i]]
        # Return the array with the updated white matter labels
        return new_label_array
    ...
    def run(cls, synthseg_array):
        ...
        Parameters
        ----------
        synthseg_array : numpy.ndarray
            Array containing SYNSEG labels.

        Returns
        -------
        numpy.ndarray
            Array with completed white matter parcellation.
...
class WhiteMatterParcellation2(WhiteMatterParcellation):
    """
    Extended class for white matter parcellation with additional SYNSEG labels.
    Resolve left-right mismatch of SYNSEG tokens.

    Attributes
    ----------
    synthseg_left_label : numpy.ndarray
        Array containing SYNSEG labels for the left hemisphere.
    synthseg_right_label : numpy.ndarray
        Array containing SYNSEG labels for the right hemisphere.
    synthseg33_left_label : numpy.ndarray
        Array containing SYNSEG33 labels for the left hemisphere.
    synthseg33_right_label : numpy.ndarray
        Array containing SYNSEG33 labels for the right hemisphere.

    Methods
    -------
    hemi_revise(synthseg_array, synthseg33_array)
        Revises SYNSEG labels based on SYNSEG33 labels.
    run(synthseg_array, *args, **kwargs)
        Runs the entire white matter parcellation process, including SYNSEG33 revision.
    ...
    def hemi_revise(cls, synthseg_array, synthseg33_array):
        ...
        synthseg_array : numpy.ndarray
            Array containing SYNSEG labels.
        synthseg33_array : numpy.ndarray
            Array containing SYNSEG33 labels.

        Returns
        -------
        numpy.ndarray
            Array with revised SYNSEG labels.
    ...
    def run(cls, synthseg_array, *args, **kwargs):
        ...
        **kwargs : dict
            Additional keyword arguments, including 'synthseg33_array'.
...
class CorpusCallosumParcellation:
    """
    Class for corpus callosum parcellation.

    Attributes
    ----------
    cc_prerequisite : dict
        Prerequisite SYNSEG labels for corpus callosum parcellation.
    lateral_ventricle : dict
        Labels for lateral ventricles.
    cc_white_matter : dict
        White matter labels for corpus callosum.
    cc_target : dict
        Target labels for corpus callosum parcellation.

    Methods
    -------
    get_cluster_min_max(prerequisite_cluster)
        Calculates minimum and maximum coordinates for clusters.
    get_prerequisite_cluster(measure_np)
        Identifies prerequisite clusters based on SYNSEG labels.
    cc_parcellation(label_array)
        Performs corpus callosum parcellation.
    _get_cc_x_y_axis_index(cc_index)
        Extracts x and y axis indices for corpus callosum.
    _get_df_lateral_ventricle(left_lateral_ventricle_index, right_lateral_ventricle_index)
        Creates a DataFrame for lateral ventricles.
    _get_df_cc(left_cc_index, right_cc_index)
        Creates a DataFrame for corpus callosum.
    cc_adapt(cc_array, label_array)
        Adapts corpus callosum labels based on lateral ventricle positions.
    run(label_array)
        Runs the corpus callosum parcellation process.
    ...
    def get_cluster_min_max(cls, prerequisite_cluster):
        ...
        Parameters
        ----------
        prerequisite_cluster : numpy.ndarray
            Array containing prerequisite clusters.

        Returns
        -------
        list
            Lists of minimum and maximum coordinates for clusters.
    ...
    def get_prerequisite_cluster(cls, measure_np):
        ...
        Parameters
        ----------
        measure_np : numpy.ndarray
            Array containing SYNSEG labels.

        Returns
        -------
        tuple
            Tuple containing prerequisite clusters and selected clusters.
    ...
    def cc_parcellation(cls, label_array):
        ...
        -------
        numpy.ndarray
            Array with corpus callosum parcellation.
    ...
    def _get_cc_x_y_axis_index(cls, cc_index):
        ...
        Parameters
        ----------
        cc_index : numpy.ndarray
            Array containing corpus callosum indices.

        Returns
        -------
        numpy.ndarray
            Array with x and y axis indices for corpus callosum.
    ...
    def _get_df_lateral_ventricle(cls, left_lateral_ventricle_index, right_lateral_ventricle_index):
        ...
        Parameters
        ----------
        left_lateral_ventricle_index : numpy.ndarray
            Array containing indices for the left lateral ventricle.
        right_lateral_ventricle_index : numpy.ndarray
            Array containing indices for the right lateral ventricle.

        Returns
        -------
        pandas.DataFrame
            DataFrame with lateral ventricle information.
    ...
    def _get_df_cc(cls, left_cc_index, right_cc_index) -> pd.DataFrame:
        ...
        Parameters
        ----------
        left_cc_index : numpy.ndarray
            Array containing indices for the left corpus callosum.
        right_cc_index : numpy.ndarray
            Array containing indices for the right corpus callosum.

        Returns
        -------
        pandas.DataFrame
            DataFrame with corpus callosum information.
    ...
    def cc_adapt(cls, cc_array, label_array):
        ...
        cc_array : numpy.ndarray
            Array containing corpus callosum labels.
        label_array : numpy.ndarray
            Array containing SYNSEG labels.
        ...
        -------
        numpy.ndarray
            Array with adapted corpus callosum labels.
    ...
    def run(cls, label_array):
        ...
        -------
        numpy.ndarray
            Array with completed corpus callosum parcellation.
...
class ECICParcellation:
    """
    Class for EC-IC parcellation.

    Attributes
    ----------
    ec_ic_parcellation_mapping : dict
        Mapping of EC-IC parcellation labels.
    ec_ic_prerequisite : dict
        Prerequisite labels for EC-IC parcellation.
    ec_ic_white_matter : dict
        White matter labels for EC-IC parcellation.
    decimal_places : int
    ...
    Methods
    -------
    ec_ic_parcellation(label_array)
        Performs EC-IC parcellation.
    run(label_array)
        Runs the EC-IC parcellation process.
...
def run(synthseg_array, depth_number):
    ...
    synthseg_array_bullseye = BullseyeProcess.run(re_white_matter_parcellation_array, synthseg_array,
...
def run_with_WhiteMatterParcellation(synthseg_array, synthseg33, depth_number):
    ...
    synthseg_array_wm = WhiteMatterParcellation2.run(synthseg_array=synthseg_array,
                                                     synthseg33_array=synthseg33_array)
    synthseg_array_cc = CorpusCallosumParcellation.run(synthseg_array_wm)
    synthseg_array_ec = ECICParcellation.run(synthseg_array_wm)
    revert_array = WhiteMatterParcellation.re_run(synthseg_array_wm, synthseg_array_cc, synthseg_array_ec)
...
def run_cmb(synthseg_array, depth_number=5):
    ...
    cmb_array = CMBProcess.run(david_label_array)
    return cmb_array
...
def run_wmh(synthseg_array, synthseg_array_wm, depth_number=5):
    ...
    out_array[cc_mask] = synthseg_array_cc[cc_mask]
    david_label_array = WhiteMatterParcellation.label_to_david_label(out_array)
    wmh_array = WMHProcess.run(david_label_array)
    return wmh_array


def run_dwi(synthseg_array, depth_number=5):
    david_label_array, synthseg_array_wm = run(synthseg_array=synthseg_array, depth_number=depth_number)
    dwi_array = DWIProcess.run(david_label_array)
    return dwi_array
...
def main(args):
    ...
    # --------- 檢 參數 建立存檔名稱 start---------------
    if args.all:
        # cmb_file_list = list(map(lambda x: x.replace('.nii.gz', f'_{args.cmb_file}.nii.gz'), file_list))
        # dwi_file_list = list(map(lambda x: x.replace('.nii.gz', f'_{args.dwi_file}.nii.gz'), file_list))
        # wmh_file_list = list(map(lambda x: x.replace('.nii.gz', f'_{args.wmh_file}.nii.gz'), file_list))
        # wm_file_list = list(map(lambda x: x.replace('.nii.gz', f'_david.nii.gz'), file_list))
    ...
    for i in range(len(file_list)):
        ...
        try:
            seg_array, synthseg_array_wm = run_with_WhiteMatterParcellation(synthseg_array=synthseg_array,
                                                                            ...
                                                                            depth_number=depth_number)
            # seg_array, synthseg_array_wm = run(synthseg_array=synthseg_array,
            #                                    depth_number=depth_number)
