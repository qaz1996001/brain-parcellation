"""
SyncService V2 with dependency injection pattern.

This module provides DCOPEventDicomServiceV2 which receives all configuration
via constructor injection instead of reading from environment variables.

Design Principles:
- Knuth: Explicit parameters, clear types, no hidden dependencies
- Linus (Data): Good data structure eliminates special cases
- Pure Function: Same inputs always produce same outputs (no side effects)
...
from backend.app.sync.schemas import (
...
class DCOPEventDicomServiceV2(BaseRepositoryService[DCOPEventModel]):
    """
    V2 DCOP Event DICOM Service with dependency injection.

    This service receives all configuration via constructor injection,
    eliminating all os.getenv() calls from method bodies.

    Design Pattern: Dependency Injection
    - Configuration passed via __init__(config: BackendConfig)
    - No environment variable reads in methods
    - Testable with mock configurations
    - Supports dual deployment (production + testing backends)

    Attributes:
        config: Immutable BackendConfig instance
    """
    ...
    async def check_study_series_transfer_complete(
        ...
        """
        Check if all series under a study have completed transfer.

        Uses self.config instead of os.getenv() for all path configurations.
        """
        logger.info(f"check_study_series_transfer_complete data {data}")

        # Get configuration from injected config (not os.getenv)
        upload_data_api_url = self.config.api.upload_data_url
        ...
        if dcop_event_list:
            await self._send_events(str(upload_data_api_url), dcop_event_dump_list)
            await self._initiate_conversion_process(
                str(upload_data_api_url),
                dcop_event_list,
                str(path_rename_dicom),
                str(path_rename_nifti),
            )
    ...
    async def _get_studies_ready_for_transfer(
        ...
        """Retrieve studies ready for transfer completion."""
        dcop_event_list = []
        dcop_event_dump_list = []
    ...
    async def _send_events(self, api_url: str, event_data: List[dict]) -> None:
        """Send study transfer complete events to the API."""
    ...
    async def study_series_nifti_tool(
        self, data: List[DCOPEventNIFTITOOLRequest]
    ) -> None:
    ...
    async def nifti_tool_get_series_info(
        ...
        for dcop_event in dcop_event_list:
            ...
            output_nifti_path = path_rename_nifti
        ...
        try:
            if dcop_event_list:
                ...
                for task_params in task_params_list:
                    ...
                    base_api_url = self.config.api.upload_data_url
                    task_dict["upload_data_api_url"] = str(base_api_url)
    ...
    def get_orthanc_study_uid_series_uid(
        self, instance_path_str: str
    ...
    def get_orthanc_series_uid(
        self, study_uid: str, series_dir_set: set
    ) -> pd.DataFrame:
    ...
    async def dicom_tool_get_series_info(
        self, data: List[DCOPEventModel]
    ) -> Optional[None]:
        ...
        for dcop_event in data:
            ...
            logger.info(
                f"dicom_tool_get_series_info study_uid_raw_dicom_path {study_uid_raw_dicom_path}"
            )
            if study_uid_raw_dicom_path.exists():
                ...
                try:
                    series_uid_list = list(
                        map(lambda x: validate_orthanc_id(x.name), series_dir_set)
                    )
                except ValueError:
    ...
    async def check_study_series_conversion_complete(
        ...
        """Check if study series conversion is complete using injected config."""
        ...
        if completed_study_events:
            ...
            await self._send_events(str(upload_data_api_url), study_events_filter)
    ...
    async def query_studies_pending_completion(
        self, study_uid: Optional[str] = None
    ) -> set:
        """Query for studies pending completion."""
        async with self.session_manager.get_session() as session:
    ...
    async def _queue_inference_tasks(
        ...
        """Queue inference tasks using injected config."""
        ...
        for dcop_event in study_events:
            ...
            dcop_event_inference_ready = DCOPEventRequest(
                ...
                params_data={
                    ...
                    "study_uid": dcop_event.study_uid,
                    "study_id": dcop_event.study_id,
                    "upload_data_api_url": str(base_api_url),
            ...
            await redis_client.set(inference_task_key, "queued", ex=21600)
            logger.info(
    ...
    def _group_series_by_study(self, events) -> Dict[str, Dict[str, Any]]:
        """Group series completion events by study."""
        series_by_study: Dict[str, Dict[str, Any]] = {}
    ...
    async def identify_completed_studies(
        ...
        """Identify studies with all series converted."""
        completed_study_events = []
        async with self.session_manager.get_session() as session:
            done_count = 0
            undone = 0
    ...
    async def get_stydy_series_ope_no_status(
        ...
        """Get study series operation status with pagination."""
        async with self.session_manager.get_session() as session:
            if study_uid is None:
                sql = text(
    ...
    async def get_stydy_ope_no_status(
        ...
        """Get study operation status with pagination."""
        async with self.session_manager.get_session() as session:
            if study_uid is None:
                sql = text(
    ...
    async def get_check_study_series_conversion_complete(
        ...
        """Get check study series conversion complete status."""
        raw_dicom_path = self.config.paths.path_raw_dicom
        rename_dicom_path = self.config.paths.path_rename_dicom
        rename_nifti_path = self.config.paths.path_rename_nifti
