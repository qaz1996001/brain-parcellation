...
class DCOPEventModel(base.DefaultBase):
    ...
    # 事件相關欄位
    kind = Column(Integer, nullable=True)  # 事件類型
    ...
    field_data = Column(String(128), nullable=True)
    ...
    async def create_event(
        ...
        if series_uid is not None:
            obj = cls(
                VsPrimaryKey=f"{tool_id}_{status}_{series_uid}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
                tool_id=tool_id,
                study_uid=study_uid,
            ...
            obj = cls(
                VsPrimaryKey=f"{tool_id}_{status}_{study_uid}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
                tool_id=tool_id,
                study_uid=study_uid,
                series_uid=series_uid,
                code_name=status,
