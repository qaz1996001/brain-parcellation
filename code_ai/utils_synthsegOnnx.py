...
class SynthSegOnnx:
    ...
    def run(
        ...
        mask_1 = np.logical_or(
            (temp_index_index == 2), (temp_index_index == 20)
        ).astype(np.float32)
        ...
        parc_inputs = {
            net_parcellation.get_inputs()[0].name: np.expand_dims(
                np.concatenate(
                    [
                        intput_im[:, 0],
                        mask_2,
