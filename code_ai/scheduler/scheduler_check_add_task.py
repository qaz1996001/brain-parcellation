...
# 設置日誌記錄器
logger = nb_log.LogManager(
    "add_raw_dicom_to_nii_inference"
...
def get_env_paths() -> Tuple[pathlib.Path, pathlib.Path, pathlib.Path]:
    ...
    if not all([path_raw_dicom, path_rename_dicom, path_rename_nifti]):
        raise ValueError(
            "環境變數缺失: 請確保設置了PATH_RAW_DICOM, PATH_RENAME_DICOM, PATH_RENAME_NIFTI"
        )
...
def is_task_exists(session: Session, **filter_args) -> bool:
    ...
    try:
        result = (
            session.query(RawDicomToNiiInference).filter_by(**filter_args).one_or_none()
        )
...
def process_nii_for_inference(
    session: Session, nifti_path: pathlib.Path, dicom_path: pathlib.Path
) -> bool:
    """處理NII文件的推理任務"""
...
def check_dicom_completeness(dicom_dir: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            if missing_tags:
                logger.warning(
                    f"DICOM文件缺少必要標籤 {', '.join(missing_tags)}: {dicom_dir}"
                )
            ...
            # 嘗試從不同的標籤獲取圖像數量信息
            if (
                hasattr(ds, "ImagesInAcquisition")
                and ds.ImagesInAcquisition is not None
            ):
                ...
                try:
                    ...
                    for f in dicom_files[:100]:  # 限制檢查文件數以提高效率
                        ...
                        if hasattr(ds_temp, "InstanceNumber"):
                            max_instance = max(
                                max_instance, int(ds_temp.InstanceNumber)
                            )
            ...
            if (
                expected_count is not None and len(dicom_files) < expected_count * 0.9
            ):  # 允許10%的誤差
                logger.warning(
                    f"DICOM文件數量不完整，預期約{expected_count}個，實際{len(dicom_files)}個: {dicom_dir}"
                )
                return False

            # 基本檢查通過
            logger.info(
                f"DICOM目錄完整性檢查通過: {dicom_dir} (檔案數: {len(dicom_files)})"
            )
            return True
...
def is_task_waiting_in_queue(
    session: Session, nifti_path: str, dicom_path: str
) -> bool:
    """
    檢查任務是否正在等待隊列中處理
    ...
    try:
        # 1. 首先查詢資料庫中是否有對應的記錄
        result = (
            session.query(RawDicomToNiiInference)
            .filter(
        ...
        try:
            ...
            connection_params = pika.ConnectionParameters(
                ...
                credentials=pika.PlainCredentials(
                    BrokerConnConfig.RABBITMQ_USER, BrokerConnConfig.RABBITMQ_PASS
                ),
            )
            ...
            while method_frame:
                try:
                    ...
                    # 檢查是否為目標任務
                    if (
                        task_body.get("nifti_study_path") == nifti_path
                        and task_body.get("dicom_study_path") == dicom_path
                    ):
...
def check_output_files_and_rerun(
    session: Session, pipeline_task: Dict[str, Any]
) -> bool:
    """
    ...
    if cmd_str:
        ...
        if not inputs_exist:
            logger.error(
                f"任務{study_id}({name})輸入文件不存在且DICOM不完整，無法重新運行"
            )
            return False
        ...
        if success:
            ...
            if (
                files_created
                and pipeline_task.get("input_list")
                and pipeline_task.get("input_dicom_dir")
            ):
                try:
                    nifti_path = pipeline_task["input_list"][0]
                    dicom_path = pipeline_task["input_dicom_dir"]
            ...
            if outputs_ok:
                ...
                for output_file in task.get("output_list", []):
                    if (
                        ...
                        and os.path.getmtime(output_file) > time.time() - 300
                    ):  # 5分鐘內創建的文件
                        rerun = True
                        break
...
def add_raw_dicom_to_nii_inference(
    ...
    try:
        ...
        study_dir_name_list = list(
            map(lambda x: os.path.basename(x.sub_dir), processed_tasks)
        )
        print(study_dir_name_list)
        post_study(study_dir_name_list)
        # 批次等待所有任務完成
        logger.info(
            f"已提交 {len(processed_tasks)} 個DICOM到NII轉換任務，等待所有任務完成..."
        )
        # processed_results = []
        # if processed_tasks:
        #     for task in processed_tasks:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tasks_file", type=str, help="任務配置文件的路徑，用於檢查特定任務"
    )
