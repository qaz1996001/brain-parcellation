...
def get_dicom_data(dicom_ds: FileDataset):
    ...
    return dict(
        ...
        study_date=studies_date,
        study_time=studies_time,
...
def list_dicom_study(data_path: pathlib.Path) -> pd.DataFrame:
    ...
    df1 = df.pivot_table(
        index=["patients_id", "study_date", "accession_number"],
        columns="series_name",
        aggfunc="count",
    )
...
def list_dicom(data_path: pathlib.Path):
    ...
    df1.to_excel(
        str(data_path.parent.joinpath(f"{data_path.name}_df_dicom.xlsx")),
        merge_cells=False,
        index=False,
    )
    df1.to_csv(
        str(data_path.parent.joinpath(f"{data_path.name}_df_dicom.csv")), index=False
    )
    df2.to_excel(
        str(data_path.parent.joinpath(f"{data_path.name}_df_series.xlsx")),
        merge_cells=False,
        index=False,
    )
    df2.to_csv(
        str(data_path.parent.joinpath(f"{data_path.name}_df_series.csv")), index=False
    )
