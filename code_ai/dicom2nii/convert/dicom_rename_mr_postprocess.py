...
class MRProcessingStrategy(ProcessingStrategy):
    ...
    def process(self, study_path: pathlib.Path, *args, **kwargs):
        series_folder_list = list(
            filter(lambda x: x.is_dir() and x.name != ".meta", study_path.iterdir())
        )
        meta_folder = study_path.joinpath(".meta")
        meta_folder.mkdir(exist_ok=True)
        for series_folder in tqdm(
            series_folder_list,
            ...
            try:
                ...
                dicom_list = list(
                    map(
                        lambda x: dcmread(str(x), force=True, stop_before_pixels=True),
                        series_folder.iterdir(),
                    )
                )
                for i in dicom_list:
                    ...
                    if result:
                        ...
                        dicom_header.update(
                            {str(i.SeriesInstanceUID): [i.to_json_dict()]}
                        )
...
class ADCProcessingStrategy(MRDicomProcessingStrategy):
    ...
    def process(self, study_path: pathlib.Path, *args, **kwargs):
        ...
        #
        filter_series_folder_list = sorted(
            filter(
                lambda x: (x.name == MRSeriesRenameEnum.ADC.value)
        ...
        if filter_series_folder_list and len(filter_series_folder_list) >= 2:
            ...
            dwi_1000_series_folder = study_path.joinpath(
                MRSeriesRenameEnum.DWI1000.value
            )
            ...
            if len(adc_series_dicom_list) == len(dwi_1000_series_dicom_list):
                ...
                # read DWI1000
                temp_dwi_list: List[Tuple[FileDataset, pathlib.Path]] = [
                    (self.read_dicom(dicom_path), dicom_path)
                ...
                dicom_list = [
                    (dcmread(x, force=True), x) for x in adc_series_dicom_list
                ]
                adc_dicom_list = list(
                    filter(
                        lambda x: not self.has_image_position_patient_with_dicom(x[0]),
                        dicom_list,
                    )
                )
                # read DWI1000
            ...
            try:
                ...
                sorted_adc_dicom_list: List[Tuple[FileDataset, pathlib.Path]] = sorted(
                    adc_dicom_list, key=lambda x: x[0][0x20, 0x13].value
                )
                ...
                sorted_dwi_1000_dicom_list: List[Tuple[FileDataset, pathlib.Path]] = (
                    sorted(dwi_dicom_list, key=lambda x: x[0][0x27, 0x1041].value)
                )

            for index, (dwi_1000_dicom_ds, dwi_1000_dicom_path) in enumerate(
                sorted_dwi_1000_dicom_list
            ):
                try:
