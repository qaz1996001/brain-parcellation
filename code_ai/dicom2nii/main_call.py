...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Thread cont .\r\nExample ： --output_nifti output_nifti_path --work 4",
    )
    parser.add_argument(
        "--upload_all",
        dest="upload_all",
        type=str,
        help="Thread cont .\r\nExample ： --upload_all True",
    )
    parser.add_argument(
        "--upload_nifti",
        dest="upload_nifti",
    ...
    parser.add_argument(
        ...
        dest="upload_dicom",
        type=str,
        help="upload rename dicom all file to NAS\r\nExample ： --upload_dicom True",
    )

    return parser.parse_args()


def run_dicom_rename_mr(work: int, input_path, output_path):
...
def run_convert_nifti(executor: ProcessPoolExecutor, output_study_set, output_path):
    from convert.convert_nifti import Dicm2NiixConverter

    converter_objects = [
        Dicm2NiixConverter(input_path=input_path, output_path=output_path)
...
if __name__ == "__main__":
    ...
    if input_dicom_path and output_dicom_path and output_nifti_path:
        ...
        output_study_set = run_dicom_rename_mr(
            work=dicom_work, input_path=input_dicom_path, output_path=output_dicom_path
        )
        ...
        if output_study_set is not None:
            nifti_output_study_set = run_convert_nifti(
                executor=convert_nifti_executor,
            ...
            print(
                "nifti_output_study_set", ",".join(str_list), "nifti_output_study_set"
            )
