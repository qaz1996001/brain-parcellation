...
@Booster('call_handler_inference_queue',
         broker_kind=BrokerEnum.RABBITMQ_AMQPSTORM, qps=5,
...
@Booster('study_task_inference_queue',
         broker_kind=BrokerEnum.RABBITMQ_AMQPSTORM, qps=5,
         concurrent_mode=ConcurrentModeEnum.SOLO,
         concurrent_num=10,
         is_send_consumer_hearbeat_to_redis=True,
         is_using_rpc_mode=True)
def study_task_inference(func_params  : Dict[str,any]):
    task_params = intput_params.StudyTaskInferenceParams.model_validate(func_params)
    mapping_inference = task_params.mapping_inference
    ...
    analyses = Analysis(study_id=base_output_path.name,**tasks)
...
def task_inference(func_params  : Dict[str,any]):
    ...
    for mapping_inference in mapping_inference_list:
        func_params = intput_params.StudyTaskInferenceParams(mapping_inference=mapping_inference,
                                                             output_study_nifti_path=output_study_nifti_path)
        study_task_inference.push(func_params.get_str_dict())
