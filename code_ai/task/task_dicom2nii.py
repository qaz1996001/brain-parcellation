...
def _execute_dcm2niix(
    ...
    """
    Core dcm2niix execution logic - synchronous, no queue dependency.

    This pure function can be called directly for synchronous conversion
    (e.g., within Series Level inference) or wrapped by @Booster for async queue.

    Args:
        series_path: Input DICOM directory path
        output_series_path: Output path base (without .nii.gz extension)
        output_series_file_path: Full output file path (with .nii.gz extension)
        timeout: Subprocess timeout in seconds (default 300s = 5min)

    Returns:
        str: Output series name on success, or error message on failure
    """
...
def call_dcm2niix(func_params: Dict[str, any]):
    ...
    This function maintains backward compatibility - same signature, same return value.
    Internal implementation now delegates to _execute_dcm2niix pure function.
    """
    task_params = intput_params.CallDcm2niixParams.model_validate(
        func_params, strict=False
    )
    return _execute_dcm2niix(
...
def get_orthanc_study_uid_series_uid(instance_path_str: str):
    ...
    client = Orthanc(
        ...
        password=ORTHANC_USER_PASSWORD,
    )
...
def process_dir(func_params: Dict[str, any]):
    ...
    for study_uid in study_uid_unique:
        series_dir_path_set = set(
            df.loc()[df["study_uid"] == study_uid, "instance_dir_path"].to_list()
        )
        for series_dir in series_dir_path_set:
            series_uid = series_dir.name
            study_id = df[df["study_uid"] == study_uid]["study_id"].iloc()[0]
            raw_dicom_path = df[df["instance_dir_path"] == series_dir][
                "instance_dir_path"
            ].iloc()[0]
            rename_dicom_path = df[df["instance_dir_path"] == series_dir][
                "rename_dicom_path"
            ].iloc()[0]
            dcop_event = DCOPEventRequest(
                study_uid=study_uid,
