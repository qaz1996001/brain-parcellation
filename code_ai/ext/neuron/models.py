    ...
    for level in range(nb_levels):
        ...
        if use_residuals:
            ...
            if nb_feats_in > 1 and nb_feats_out > 1 and (nb_feats_in != nb_feats_out):
                ...
                if conv_dropout > 0:
                    noise_shape = [None, *[1] * ndims, nb_lvl_feats]
                    convarm_layer = KL.Dropout(conv_dropout, noise_shape=noise_shape)(
                        last_tensor
                    )
...
def add_prior(
    ...
    if use_logp:
        print(
            "Breaking change: use_logp option now requires log input!", file=sys.stderr
        )
        merge_op = KL.add
...
def single_ae(
    ...
    if ae_type == "dense":
        ...
        if (
            ...
            last_tensor = convL(enc_size[-1], conv_size, name=name, **conv_kwargs)(
                pre_enc_layer
            )
    ...
    if ae_type == "dense":
        ...
        name = "%s_ae_%s_dec" % (prefix, ae_type)
        last_tensor = convL(input_nb_feats, conv_size, name=name, **conv_kwargs)(
            last_tensor
        )
