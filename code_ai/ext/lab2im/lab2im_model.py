...
def lab2im_model(
    ...
    stds_input = KL.Input(
        shape=list(generation_labels.shape) + [n_channels], name="stds_input"
    )
    ...
    image = layers.BiasFieldCorruption(0.3, 0.025, same_bias_for_all_channels=False)(
        image
    )

    # intensity augmentation
    image._keras_shape = tuple(image.get_shape().as_list())
    image = layers.IntensityAugmentation(clip=300, normalise=True, gamma_std=0.2)(image)
    ...
    brain_model = Model(
        inputs=[labels_input, means_input, stds_input], outputs=[image, labels]
    )
            ...
            if output_shape != tmp_shape:
                print(
                    "output shape {0} not divisible by {1}, changed to {2}".format(
                        output_shape, output_div_by_n, tmp_shape
                    )
                )
        ...
        if output_div_by_n is not None:
            output_shape = [
                utils.find_closest_number_divisible_by_m(
                    s, output_div_by_n, answer_type="closer"
                )
                for s in output_shape
            ]
