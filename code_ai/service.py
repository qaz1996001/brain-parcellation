...
#
#     BENTOML_MODEL1_TAG = "MP-input64-aug_rot_bc10-bz32-unet5_32-bce_dice-Adam1e3_cosine"
#     BENTOML_MODEL2_TAG = "MP-norm1-input26ch2_gauss_loc-aug2-bz64-Res32x3FPN-D128x4D1-cw"
#
#     # bento_model1_ref = bentoml.tensorflow.get(BENTOML_MODEL1_TAG)
#     # bento_model2_ref = bentoml.tensorflow.get(BENTOML_MODEL2_TAG)
#
...
#         import tensorflow as tf
#         self.bento_model1_ref = bentoml.models.get(self.BENTOML_MODEL1_TAG)
#         self.bento_model2_ref = bentoml.models.get(self.BENTOML_MODEL2_TAG)
#         self.model1 = tf.saved_model.load(self.bento_model1_ref.path)
#         self.model2 = tf.saved_model.load(self.bento_model2_ref.path)
...
#     @bentoml.api
#     async def classify(self, swan_path_str: str,
#                        temp_path_str: str,
#                        out_root_str: str) -> str:
#         '''
...
#             self.save_nii_trio(swan_path, pred_label, out_root=str(out_root), name_rule=self.name_rule)
#             df_label = self.save_label_table(swan_path, df_pred, pred_label, seg_arr,
#                                              out_root=str(out_root), name_rule=self.name_rule)
#
#         return str(out_root)
#
#     @classmethod
...
#
#     @classmethod
#     def save_nii_trio(cls, nifti_file, pred_label, save_image=True, out_root='test_label_out',
#                       seg_dir='synthseg_out_temp',
#                       name_rule='file', verbose=False):
#         # name output folder
...
#         nib.save(x, f"{out_dir}/{nifti_file.name.replace('.nii.gz', '')}_Pred_CMB.nii.gz")
#         if verbose:
#             print(
#                 f"size={x.header.get_data_shape()} spacing={x.header.get_zooms()}  {label_arr.dtype}:{label_arr.min()}-{label_arr.max()}")
#             print("affine matrix =\n", x.affine)
#     @classmethod
#     def save_label_table(cls, nifti_file, df_pred, label_arr, seg_arr, out_root='test_label_out', name_rule='file'):
#         def get_location(lb):
