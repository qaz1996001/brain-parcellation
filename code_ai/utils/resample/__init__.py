...
def resampleSynthSEG2original_0204(
...
def get_original_z_index(img_array, z_i, img_1mm_ori, z_i1, z_pixdim_img):
    if z_i < 64:
        ...
        for z_index in range(z_i):
            img_array_broadcast = np.broadcast_to(
                np.expand_dims(img_array[:, :, z_index], -1), broadcast_shape
            )
...
def check_original_z_index(argmin, img_array, img_1mm_ori, z_pixdim_img):
    ...
    if np.any(diff_index < 0):
        ...
        for i in error_index:
            ...
            img_array_broadcast = np.broadcast_to(
                np.expand_dims(img_array[:, :, img_index], -1), broadcast_shape
            )
            diff = np.sum(
                np.abs(img_array_broadcast - img_1mm_ori[:, :, start_i:end_i]),
                axis=(0, 1),
            )
...
def resampleSynthSEG2original(
    ...
    img_nii, img_array, y_i, x_i, z_i, header_img, pixdim_img = get_volume_info(
        str(raw_file)
    )
    img_1mm_nii, img_1mm_array, y_i1, x_i1, z_i1, header_img_1mm, pixdim_img_1mm = (
        get_volume_info(str(resample_image_file))
    )
...
def save_original_seg_by_argmin_z_index(
    raw_file: pathlib.Path, resample_seg_file: pathlib.Path, argmin
):
    img_nii, img_array, y_i, x_i, z_i, header_img, pixdim_img = get_volume_info(
        str(raw_file)
    )
    img_1mm_nii, img_1mm_array, y_i1, x_i1, z_i1, header_img_1mm, pixdim_img_1mm = (
        get_volume_info(str(resample_seg_file))
    )
...
def save_nii_original_seg_by_argmin(
    raw_file: pathlib.Path, resample_seg_file: pathlib.Path, argmin
):
    img_nii, img_array, y_i, x_i, z_i, header_img, pixdim_img = get_volume_info(
        str(raw_file)
