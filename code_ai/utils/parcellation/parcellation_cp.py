...
# Import skimage separately as it doesn't have CuPy equivalent
...
def asarray(array):
    """Convert to backend array format."""
    return xp.asarray(array)


def measure_label(array, connectivity=2):
    """Wrapper for measure.label that works with both backends."""
    # Always use CPU for measure.label as it's not available in CuPy
    # array_np = asnumpy(array)
    result = ndimage.label(array)
    return asarray(result) if HAS_CUPY else result


# Modified pairwise_distance function
def pairwise_distance(a, b):
    ...
    """
    return xp.sum(xp.abs(a - b) ** 2, axis=-1)


# Modified loss_distance function
def loss_distance(index_sub, label_index_sub, decimal_places=8):
    ...
    Parameters
    ----------
    index_sub : array_like
        Subset of points
    label_index_sub : array_like
        Subset of label points
    ...
    index_sub = asarray(index_sub)
    label_index_sub = asarray(label_index_sub)

    # Create expanded arrays for broadcasting
    XA = xp.expand_dims(index_sub, axis=1)
    XB = xp.expand_dims(label_index_sub, axis=0)

    # Tile to create full comparison matrices
    XA = xp.tile(XA, (1, XB.shape[1], 1))
    XB = xp.tile(XB, (XA.shape[0], 1, 1))
...
class WhiteMatterParcellation:
    ...
    def white_matter_parcellation(cls, label_array):
        ...
        for k in cls.cerebral_white_matter:
            # 2 or 41
            index = xp.argwhere(label_array == cls.cerebral_white_matter[k])
            white_matter_mapping_keys = list(cls.white_matter_mapping[k].keys())
            np_loss = xp.zeros((index.shape[0], len(white_matter_mapping_keys)))
            np_loss[:, :] = 999999
            for i in range(len(white_matter_mapping_keys)):
                # 3001 ... 4007
                label_index = xp.argwhere(label_array == white_matter_mapping_keys[i])
                loss_list = []
                index_sub_arg_list = []
                # Z 軸切片
                ...
                if len(index_sub_arg_list) > 0:
                    index_sub_arg = xp.concatenate(index_sub_arg_list)
                    np_loss[index_sub_arg, i] = xp.concatenate(loss_list).reshape(-1, 1)
                else:
                    continue
...
class WhiteMatterParcellation2(WhiteMatterParcellation):
    ...
    def hemi_revise(cls, synthseg_array, synthseg33_array):
        ...
        kk = 3
        while xp.logical_or(xp.any(temp_array == 100), xp.any(temp_array == 200)):
            kk += 1
            window_size = kk
            half_window = window_size // 2
            temp_array_index = xp.argwhere(temp_array == 100)
    ...
    def get_cluster_min_max(cls, prerequisite_cluster):
        ...
        for i in prerequisite_cluster_unique:
            if i == 0:
                ...
                y_max = xp.argwhere(prerequisite_cluster == i)[:, 0].max()
                y_min = xp.argwhere(prerequisite_cluster == i)[:, 0].min()
                x_max = xp.argwhere(prerequisite_cluster == i)[:, 1].max()
                x_min = xp.argwhere(prerequisite_cluster == i)[:, 1].min()
...
class BullseyeProcess:
    ...
    def filter_labels(cls, in0, include_superlist, fixed_id=None, map_pairs_list=None):
        ...
        out0 = xp.zeros(in0.shape, dtype=in0.dtype)
    ...
    def norm_dist_map(cls, orig, dest):
        ...
        dist_orig = distance_transform_edt(xp.logical_not(orig.astype(xp.bool_)))
        dist_dest = distance_transform_edt(xp.logical_not(dest.astype(xp.bool_)))
    ...
    def merge_labels(cls, in1, in2, intersect=False):
        ...
        # if intersection, create new label-set as cartesian product of the two sets
            ...
            u1_set = xp.unique(in1.ravel())
            u2_set = xp.unique(in2.ravel())
    ...
    def generate_wmparc(cls, incl_aux, ndist, label, incl_labels=None, verbose=False):
        ...
        if incl_labels is None:
            ...
            incl_mask = xp.zeros(incl_aux.shape, dtype=xp.bool_)
    ...
    def bullsey_hemi_parcellation(cls, depth_array, synthseg_array):
        # 輸出 nii.gz 的新 array
        new_label_array = xp.zeros_like(depth_array, dtype=depth_array.dtype)
    ...
    def bullseye_depth_merge(cls, bullsey_parcellation_array, synthseg_array):
        ...
        for k in cls.lateral_ventricle:
            ...
            bullseye_cluster = xp.unique(cls.bullseye_parcellation[k])
...
class CMBProcess:
    ...
    def brain_stem_parcellation(cls, label_array):
        ...
        new_label_array = xp.zeros_like(label_array)
        # 取出 brain stem 部分
        index = xp.argwhere(label_array == cls.BRAIN_STEM)
        # 建立 誤差(距離)矩陣
...
class DWIProcess:
    ...
    def DPWM_parcellation(cls, label_array, target='frontal'):
        ...
        for k in target_dict:
            # 取出目標索引
            index = xp.argwhere(label_array == target_dict[k])
            # 取出條件上邊界索引
            temp_index = xp.argwhere(label_array == prerequisite_dict[k][0])
            temp_index_z = xp.unique(temp_index[:, 2])
            parcellation_higher = temp_index_z.max()
            # 取出條件下邊界索引
            temp_index = xp.argwhere(label_array == prerequisite_dict[k][1])
            temp_index_z = xp.unique(temp_index[:, 2])
    ...
    def brain_stem_hemi_parcellation(cls, label_array):
        ...
        index = xp.argwhere(xp.isin(label_array, BRAIN_STEM_mapping_list))
        z_aixs_intersect = xp.unique(index[:, 2])
    ...
    #     # to  pons、 medulla
    #     # brain-stem - midbrain
    #     index = xp.argwhere(new_label_array == cls.BRAIN_STEM)
    #     values_x, count_x = xp.unique(index[:, 0], return_counts=True)
    #     values_p = values_x[::-1]
    ...
    #
    #     #2023 10 24 改
    #     y_axis = xp.unique(temp_index[:, 1])
    #     y_axis_15 = xp.percentile(y_axis, 15)
    #     y_axis_85 = xp.percentile(y_axis, 85)
    ...
    #         if vector.shape[0] > 1:
    #             norm_vector.append([i, xp.ptp(vector), vector.max(), vector.min()])
    #     norm_vector = xp.array(norm_vector, dtype=xp.float64)
    #     norm_vector = xp.round(norm_vector, 8)
    #
    #     z_1_gradient = xp.gradient(norm_vector[:, 1])
    #     z_2_gradient = xp.gradient(z_1_gradient)
    #     y_index = norm_vector[z_2_gradient.argmax(), 0]
    #     z_index = index[(index[:, 0] == x_index) & (index[:, 1] == y_index), 2].min()
    #
    #     ###### 2023 10 24 改
    #     z_axis = xp.unique(temp_index[:, 2])
    #     z_norm_vector = []
    #     for i in z_axis:
    #         vector = temp_index[temp_index[:, 2] == i][:, 1]
    #         if vector.shape[0] > 1:
    #             z_norm_vector.append([i, xp.ptp(vector), vector.max(), vector.min()])
    #     z_norm_vector = xp.array(z_norm_vector, dtype=xp.float64)
    #     z_norm_vector = xp.round(z_norm_vector, 8)
    #
    #     z_1_gradient = xp.gradient(z_norm_vector[:, 1])
    #     z_2_gradient = xp.gradient(z_1_gradient)
    #     z_index = xp.where(z_2_gradient == z_2_gradient.max())[0][-1]
    ...
    def brain_stem_parcellation(cls, label_array):
        ...
        z_2_gradient = xp.gradient(z_1_gradient)
        z_index = xp.where(z_2_gradient == z_2_gradient.max())[0][-1]
        # print(rf'z_index {xp.where(z_2_gradient == z_2_gradient.max())[0]}')
    ...
    def left_right_translate(cls, slice):
        index_112 = xp.argwhere(slice == 112)
        index_212 = xp.argwhere(slice == 212)
        if index_112[:, 0].min() < index_212[:, 0].min():
            return False, slice
        else:
            return True, xp.flip(slice, 0)

    @classmethod
    def inverse_left_right_translate(cls, flip, slice):
        if flip:
            return xp.flip(slice, 0)
        else:
            return slice
...
def data_translate(slice):
    ...
    slice = xp.swapaxes(slice, 0, 1)
    # TMU scans need to be flipped
    slice = xp.flip(slice, 0)
    slice = xp.flip(slice, 1)
    return slice


def inverse_data_translate(slice):
    """Flip and swap axes for TMU scans."""
    slice = xp.swapaxes(slice, 1, 0)
    # TMU scans need to be flipped
    slice = xp.flip(slice, 0)
    slice = xp.flip(slice, 1)
    return slice
...
def left_right_translate(slice):
    ...
    index_3001 = xp.argwhere(slice == 3001)
    index_4001 = xp.argwhere(slice == 4001)
