...
class WhiteMatterParcellation:
    ...
    def re_run(cls, synthseg_array_wm, synthseg_array_cc, synthseg_array_ec):
        ...
        for k in CorpusCallosumParcellation.cc_white_matter:
            ...
            diff_mask = np.logical_and(base_mask, np.logical_not(target_mask))
            out_array[diff_mask] = WhiteMatterParcellation.cerebral_white_matter[k]
        for k in ECICParcellation.ec_ic_white_matter:
            ec_ic_target_list = list(
                set(ECICParcellation.ec_ic_parcellation_mapping[k].values())
            )
...
class CorpusCallosumParcellation:
    ...
    def get_prerequisite_cluster(cls, measure_np):
        ...
        if prerequisite_cluster_unique.shape[0] > 2:
            ...
            prerequisite_cluster = np.where(
                np.isin(prerequisite_cluster, del_cluster), 0, prerequisite_cluster
            )
    ...
    def cc_parcellation(cls, label_array):
        ...
        for k in cls.cc_white_matter:
            ...
            prerequisite_start_index = np.argwhere(
            ...
            z_aixs_intersect = np.intersect1d(
    ...
    def _get_df_lateral_ventricle(
        ...
        z_axis_intersect = np.intersect1d(
        ...
        df_lateral_ventricle = pd.DataFrame(
            intersect_lateral_ventricle_array,
            columns=[
                "lateral_ventricle_z",
    ...
    def _get_df_cc(cls, left_cc_index, right_cc_index) -> pd.DataFrame:
        ...
        intersect_cc_array = np.concatenate(
            [
                z_axis_intersect,
                left_cc_axis_vector[
                    np.isin(left_cc_axis_vector[:, 0], z_axis_intersect)
                ][:, 1:],
                right_cc_axis_vector[
                    np.isin(right_cc_axis_vector[:, 0], z_axis_intersect)
                ][:, 1:],
            ],
            axis=1,
        )
    ...
    def cc_adapt(cls, cc_array, label_array):
        ...
        left_lateral_ventricle_index = np.argwhere(
            label_array == cls.lateral_ventricle["left_hemi"]
        )
        right_lateral_ventricle_index = np.argwhere(
...
class ECICParcellation:
    ...
    def ec_ic_parcellation(cls, label_array):
        ...
        for k in cls.ec_ic_white_matter:
            ...
            prerequisite_end_index = np.argwhere(
            ...
            for i in range(len(ec_ic_parcellation_mapping_keys)):
                label_index = np.argwhere(
                    label_array == ec_ic_parcellation_mapping_keys[i]
                )
                loss_list = []
                index_sub_arg_list = []
                for j in np.unique(label_index[:, 2]):
            ...
            for i in np.unique(new_label):
                ...
                new_label_array[
                    ...
                    ec_ic_parcellation_mapping_keys[i]
                ]
        return new_label_array
...
class BullseyeProcess:
    ...
    def create_shells(cls, ndist, mask, n_shells=4):
        ...
        limits = np.linspace(0.0, 1.0, n_shells + 1)
    ...
    def generate_wmparc(cls, incl_aux, ndist, label, incl_labels=None, verbose=False):
        ...
        proc_mask = np.logical_and(np.logical_and(ndist > 0.0, ndist < 1.0), incl_mask)
        ...
        while not np.all(DONE_mask[proc_mask]):
            ...
            while True:
                ...
                TODO_mask = np.logical_and(
                    np.logical_and(aux, proc_mask), np.logical_not(DONE_mask)
                )
            ...
            for idx in Idx_TODO[I_sort[::-1]]:
                ...
                for off in (
                    np.argwhere(iterate_structure(connectivity, its_conn)) - its_conn
                ):
                    try:
    ...
    def bullseye_depth_merge(cls, bullsey_parcellation_array, synthseg_array):
        ...
        for k in cls.lateral_ventricle:
            lateral_ventricle_index = np.argwhere(
    ...
    def bullseye_depth_synthseg_label(
        ...
        for k in u1:
            ...
            out_mask = np.logical_or(
                np.logical_and(mask1, mask2), np.logical_and(mask1, mask41)
            )
            if (k == 3011) or (k == 4011):
...
class CMBProcess:
    ...
    def brain_stem_parcellation(cls, label_array):
        ...
        for k in cls.brain_stem_hemi_parcellation:
            ...
            label_index = np.argwhere(
                np.isin(label_array, hemi_parcellation_mapping_keys)
            )
            loss_list = []
            index_sub_arg_list = []
            # Z 軸切片
    ...
    def ventral_DC_parcellation(cls, label_array):
        ...
        for k in cls.ventral_DC:
            ...
            prerequisite_index = np.argwhere(
        ...
        for k in cls.bullseye_DPWM_parcellation:
            ...
            for ii in np.unique(new_label):
                ...
                new_label_array[
                    ...
                    hemi_parcellation_mapping_keys[ii]
                ]
        return new_label_array
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--DWIFile", "--dwiFile", type=str, default="DWI", help="DWI Mask file name."
    )
    parser.add_argument(
        "--WMH",
        "--wmh",
    ...
    parser.add_argument(
        "--WMHFile", "--wmhFile", type=str, default="WMH", help="WMH Mask file name."
    )
    parser.add_argument(
        "--depth_number",
        type=int,
