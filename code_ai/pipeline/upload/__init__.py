...
def upload_json(ID: str, mode: InferenceEnum) -> object:
    ...
    cmd_json_path_path = os.path.join(
        path_process, "Deep_cmd_tools", "{}_cmd.json".format(ID)
    )
    if os.path.exists(cmd_json_path_path):
        with open(cmd_json_path_path, "r") as f:
            data_json = json.load(f)
        cmd_data = next(
            filter(lambda x: x["study_id"] == ID and x["name"] == mode, data_json)
        )
        if cmd_data is not None:
            ...
            platform_json_list = list(
                map(
                    lambda x: str(x).replace(".nii.gz", "_platform_json.json"),
                    nii_file_list,
                )
            )

            platform_json_list = list(
                filter(lambda x: os.path.exists(x), platform_json_list)
            )
