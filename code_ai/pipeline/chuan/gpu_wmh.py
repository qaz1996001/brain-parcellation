...
def model_predict_wmh(
    path_code, path_process, model_wmh_file, case_name, path_log, gpu_n
):
    # 以log紀錄資訊，先建置log
    ...
    try:
        ...
        if gpumRate < 0.999:
            ...
            outputs = Conv2D(
                filters=1, kernel_size=(1, 1), kernel_initializer="glorot_normal"
            )(dec3)
            ...
            X = np.load(
                os.path.join(path_test_img, "npy", case_name + "_T2FLAIR.npy")
            )  # 讀出image的array矩陣，用nii讀影像的話會有背景問題

            # 只要做WMH AI，不用跑去頭皮
            Y_pred = model_wmh.predict(
                X, batch_size=1
            )  # Y_pred.shape => 23*512*512*1  512 512 23 1
            ...
            img_nii = nib.load(
                os.path.join(path_nii, case_name + "_T2FLAIR.nii.gz")
            )  # 讀取影像
...
if __name__ == "__main__":
    ...
    path_process = os.path.join(
        "/data/pipeline/chuan/process/Deep_WMH", case_name
