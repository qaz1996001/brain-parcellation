...
def class_studydescriptoion(path_in, path_out, log_file):
    ...
    for i in range(len(file_list)):
        ...
        dcm_studyid = dicom_slice[
            0x0020, 0x000D
        ].value  # StudyInstanceUID <- 以此為資料夾命名
        dcm_series = dicom_slice[
            0x08, 0x103E
        ].value  # Series Description <- 以此為資料夾命名
        dcm_series = dcm_series.replace(
            ":", " "
        )  # 把冒號替換成空格，因為冒號不能當作資料夾名稱
        dcm_series = dcm_series.replace(
            "^", " "
        )  # 把冒號替換成空格，因為冒號不能當作資料夾名稱
        dcm_series = dcm_series.replace(
        ...
        if not os.path.isdir(
            os.path.join(path_out, dcm_studyid, "img_classification", dcm_series)
        ):
            os.mkdir(
                os.path.join(path_out, dcm_studyid, "img_classification", dcm_series)
            )
        ...
        try:
            shutil.copy(
                os.path.join(path_in, file_list[i]),
                os.path.join(
                    path_out,
                    dcm_studyid,
                    "img_classification",
            ...
            logging.warning(
                "Retry!!! Image:" + file_list[i] + " can not be copied. ", exc_info=True
            )
    # 最後直接由資料夾去紀錄有幾個人
    case_name = os.listdir(path_out)
    patientid = (
        str(dcm_patientid).rjust(8, "0") + "_" + str(dcm_studydate).rjust(8, "0")
    )
...
def move_ADC_DWI_T2FLAIR(path_class, path_dcm, case_name, patientid, log_file):
    ...
    series_nopred = [
        s for s in series_list if s.lower().find("pred") < 0
    ]  # 先把有Pred的去除
    series_nopred = [
        s for s in series_nopred if s.lower().find("report") < 0
    ]  # 先把有report的去除
    series_nopred = sorted(series_nopred)
    # 以下針對series list去分類出adc, dwi0, dwi100, t2flair的資料夾
    # 偵測下方ADC字串
    matching = [
        s for s in series_nopred if s.lower().find("adc") >= 0
    ]  # 找出名稱有ADC的資料夾，ADC因為沒有其他確認方法，當第一個是因為第一個字串最短
    matching1 = [
        s
        for s in series_nopred
        if s.lower().find("apparent diffusion coefficient") >= 0
    ]  # 找出名稱有ADC的資料夾，ADC因為沒有其他確認方法，當第一個是因為第一個字串最短
    matching = sorted(matching)
    if len(matching) > 1:
        # 複數以上只能無腦以第一個複製
        path_ADC = os.path.join(
            path_class, matching[0]
        )  # 沒辦法確定有幾個符合ADC，只能無腦當作第一個
        ...
        logging.warning(
            ...
            + " "
            + str(case_name)
            + " apparent diffusion coefficient too much."
        )
    elif len(matching) == 0 and len(matching1) == 1:
        path_ADC = os.path.join(
            path_class, matching1[0]
    ...
    matching = [
        s for s in series_nopred if s.lower().find("dwi") >= 0
    ]  # 找出名稱有DWI的資料夾，DWI一定有Ax? 不確定
    matching1 = [
        s for s in series_nopred if s.lower().find("autodiff") >= 0
    ]  # 找出名稱有DWI的資料夾，DWI一定有Ax? 不確定
    matching = sorted(matching)
    ...
    if len(matching) > 1:
        ...
        logging.warning(
            "!!! " + patientid + " " + str(case_name) + " autodiff too much."
        )
    elif len(matching) == 0 and len(matching1) == 1:
        path_DWI = os.path.join(path_class, matching1[0])  # 剛好名稱為autodiff
    ...
    if os.path.isdir(path_DWI):
        ...
        for j in range(len(file_list)):
            ...
            if dwi_tag == 0:
                ...
                shutil.copy(
                    os.path.join(path_DWI, file_list[j]),
                    os.path.join(path_dcmDWI0, file_list[j]),
                )
            elif dwi_tag == 1000:
                ...
                shutil.copy(
                    os.path.join(path_DWI, file_list[j]),
                    os.path.join(path_dcmDWI1000, file_list[j]),
                )
            else:
                logging.error(
                    "!!! "
                    ...
                    + " "
                    + str(case_name)
                    + " DWI image has tag neither b=0 nor b=1000."
                )
    ...
    matching = [
        s for s in series_nopred if not s.lower().find("t1") >= 0
    ]  # 找出名稱沒有T1的資料夾
    if len(matching) > 0:
        matching1 = [
            s for s in matching if s.lower().find("flair") >= 0
        ]  # 找出名稱有FLAIR的資料夾
        if len(matching1) > 1:
            matching2 = [
                s for s in matching1 if s.lower().find("t2") >= 0
            ]  # 找出名稱有T2的資料夾
            matching2 = sorted(matching2)
            if len(matching2) > 1:
                path_T2FLAIR = os.path.join(
                    path_class, matching2[0]
                )  # 沒辦法確定有幾個符合T2FLAIR，只能無腦當作第一個
                shutil.copytree(path_T2FLAIR, path_dcmT2FLAIR)
                logging.warning(
                    "!!! " + patientid + " " + str(case_name) + " T2 FLAIR too much."
                )
            ...
            path_T2FLAIR = os.path.join(
                path_class, matching1[0]
            )  # 只有這筆有FLAIR字串，所以當作T2 FLAIR(還是有機會T1...)
            shutil.copytree(path_T2FLAIR, path_dcmT2FLAIR)
    return
...
def move_CTich(path_class, path_dcm, case_name, patientid, log_file):
    ...
    matching = sorted(
        [s for s in series_nopred if s.lower().find("c-") >= 0]
    )  # 找出名稱沒有T1的資料夾
    if len(matching) > 1:
        # 只能當作名稱最短是c-了
        path_CT = os.path.join(
            path_class, matching[0]
        )  # 沒辦法確定有幾個符合T2FLAIR，只能無腦當作第一個
        ...
        # 先找有沒有axi
        matching1 = sorted(
            [s for s in matching if s.lower().find("axi") >= 0]
        )  # 找出名稱有FLAIR的資料夾
        if len(matching1) > 1:
            path_CT = os.path.join(
                path_class, matching1[0]
...
def check_tag_num_stroke(path_dcm, case_name, patientid, log_file):
    ...
    if len(DWI1000_list) == 1:
        ...
        dcm = pydicom.dcmread(
            os.path.join(path_dcmDWI1000, file_list[0])
        )  # 讀取第一張確認影像總張數
        ImagesAcquisition = dcm[0x0020, 0x1002].value  # 得知Images in Acquisition
        if len(file_list) > ImagesAcquisition / 2:
            ans.append(2)
            logging.error(
                "!!! " + patientid + " " + str(case_name) + " DWI1000 slice too much."
            )
        elif len(file_list) == ImagesAcquisition / 2:
    ...
    if len(ADC_list) == 1 and len(DWI0_list) == 1 and len(DWI1000_list) == 1:
        ...
        if len(file_list) > ImagesAcquisition / 2:
            ans.append(2)
            logging.error(
                "!!! " + patientid + " " + str(case_name) + " ADC slice too much."
            )
        elif len(file_list) == ImagesAcquisition / 2:
            ADC_n = len(file_list)
        else:
    ...
    if len(T2FLAIR_list) == 1:
        ...
        dcm = pydicom.dcmread(
            os.path.join(path_dcmT2FLAIR, file_list[0])
        )  # 讀取第一張確認影像總張數
        ImagesAcquisition = dcm[0x0020, 0x1002].value  # 得知Images in Acquisition
        if len(file_list) > ImagesAcquisition:
            ans.append(2)
            logging.error(
                "!!! " + patientid + " " + str(case_name) + " T2FLAIR slice too much."
            )
...
def check_tag_num_ct(path_dcm, case_name, patientid, log_file):
    ...
    if len(CT_list) == 1:
        ...
        DCM_list = [
            [y, pydicom.dcmread(os.path.join(path_dcmCT, y))[0x20, 0x0032].value[-1]]
            for y in file_list
        ]  # (0020,0032) Image Position (Patient) [-119.600,-124.458,89.460]
...
def dcm2nii(path, case_name, tag):
    ...
    for j in range(len(tag)):
        bash_line = (
            "dcm2niix -v 0 -z y -o " + path_nii + " " + os.path.join(path, tag[j])
        ...
        os.rename(
            os.path.join(path_nii, [y for y in nii_list if tag[j] in y][0]),
            os.path.join(path_nii, new_name),
...
def nii_info_DWI1000_for_ADC(path, case_name):
    ...
    img_true = nib.load(
        os.path.join(path, case_name + "_" + nii_header_true + ".nii.gz")
    )  # DWI1000 nii
    img_fake = nib.load(
        os.path.join(path, case_name + "_" + nii_header_fake + ".nii.gz")
    )  # ADC nii
...
def resampleSynthSEG2original(path, case_name, series, SynthSEGmodel):
    ...
    img_1mm_nii = nib.load(
        os.path.join(path_nii, case_name + "_" + series + "_resample.nii.gz")
    )
    img_1mm_array = np.array(img_1mm_nii.dataobj)
    SynthSEG_1mm_nii = nib.load(
        os.path.join(
            path_nii, case_name + "_" + series + "_" + SynthSEGmodel + ".nii.gz"
        )
    )  # 230*230*140
    ...
    img_1mm_256_nii = nibabel.processing.conform(
        img_1mm_nii,
        ((y_i, x_i, z_i1)),
    ...
    nib.save(
        img_1mm_256_nii2,
        os.path.join(path_nii, case_name + "_" + series + "_resample_256.nii.gz"),
    )
    # 再將SynthSEG從230*230*140轉成256*256*140
    SynthSEG_1mm_256_nii = nibabel.processing.conform(
        SynthSEG_1mm_nii,
        ((y_i, x_i, z_i1)),
        (pixdim_img[1], pixdim_img[2], pixdim_img_1mm[3]),
        order=0,
    ...
    nib.save(
        SynthSEG_1mm_256_nii2,
        os.path.join(
            path_nii, case_name + "_" + series + "_" + SynthSEGmodel + "_256.nii.gz"
        ),
    )
    ...
    nib.save(
        new_SynthSeg_nii,
        os.path.join(
            path_nii, case_name + "_NEW_" + series + "_" + SynthSEGmodel + ".nii.gz"
        ),
    )
    return new_array
...
def make_SynthSEG_Brain_mask(path, case_name):
    ...
    if not os.path.isdir(path_mask):  # 如果資料夾不存在就建立
        os.mkdir(path_mask)
    img_true_header = nib.load(
        os.path.join(path_nii, case_name + "_NEW_DWI0_DWI.nii.gz")
    )
    img_array = np.array(img_true_header.dataobj)  # 讀出image的array矩陣
    img_array[img_array < 0] = 0
    img_array[img_array > 0] = 1
...
def make_BET_ADC_DWI1000(path, case_name):
    ...
    for j in range(len(key)):
        img_nii = nib.load(
            os.path.join(path, "nii", case_name + "_" + key[j] + ".nii.gz")
        )  # 讀取影像 -7 => DWI0.nii.gz
        img_array = np.array(img_nii.dataobj)  # 讀出image的array矩陣
        label_nii = nib.load(
            os.path.join(path, "BET_mask", case_name + ".nii.gz")
        )  # DWI0 used BET image
        ...
        nib.save(
            new_img, os.path.join(path_BET_img, case_name + "_" + key[j] + ".nii.gz")
        )
    return


def custom_normalize_1(
    volume,
    mask=None,
    new_min=0.0,
    new_max=1.0,
...
def ADC_DWI1000_nor(path_dcm, case_name, path_test_img):
    ...
    ADC_nii = nib.load(
        os.path.join(path_dcm, "BET_img", case_name + "_ADC.nii.gz")
    )  # 讀取影像
    ADC_array = np.array(ADC_nii.dataobj).astype(float)  # 讀出image的array矩陣
    DWI1000_nii = nib.load(
        os.path.join(path_dcm, "BET_img", case_name + "_DWI1000.nii.gz")
    )  # 讀取影像
    DWI1000_array = np.array(DWI1000_nii.dataobj).astype(float)  # 讀出image的array矩陣
    BET_nii = nib.load(
        os.path.join(path_dcm, "BET_mask", case_name + ".nii.gz")
    )  # 讀取影像
...
def T2FLAIR_nor(path_nii, path_test_img, case_name):
    ...
    # 讀取nifti檔
    T2FLAIR_nii = nib.load(
        os.path.join(path_nii, "nii", case_name + "_T2FLAIR.nii.gz")
    )  # 讀取影像
...
def make_clinical_study_csv(path_in, path_out, path_csv):
    ...
    # 接下來開始做成csv當複製影像整理出來
    DCM_list = [
        [y, pydicom.dcmread(os.path.join(path_in, "C-", y))[0x20, 0x0032].value[-1]]
        for y in DCMs
    ]  # (0020,0032) Image Position (Patient) [-119.600,-124.458,89.460]
    Position = np.array([y[-1] for y in DCM_list])
    sortP = np.sort(Position)
    ...
    for i in range(len(DCMs)):
        ...
        Position0 = DCM[0x20, 0x0032].value[
            0
        ]  # (0020,0032) Image Position (Patient) [-131.000,-122.237,51.914]
        Position1 = DCM[0x20, 0x0032].value[
            1
        ]  # (0020,0032) Image Position (Patient) [-131.000,-122.237,51.914]
        Position2 = DCM[0x20, 0x0032].value[
            2
        ]  # (0020,0032) Image Position (Patient) [-131.000,-122.237,51.914]
        Orientation0 = DCM[
            0x20, 0x0037
            ...
            0
        ]  # (0020,0037) Image Orientation (Patient) [1.000000,0.000000,0.000000,0.000000,0.976296,-0.216440]
        Orientation1 = DCM[
            0x20, 0x0037
        ].value[
            1
        ...
        PixelSpacing0 = DCM[0x28, 0x0030].value[
            0
        ]  # (0028,0030) Pixel Spacing [0.488281,0.488281]
        PixelSpacing1 = DCM[0x28, 0x0030].value[
            1
        ]  # (0028,0030) Pixel Spacing [0.488281,0.488281]
    ...
    text_df = pd.DataFrame(
        text[1:], columns=text[0]
    )  # 我们将 data[0] 作为 columns 参数传递给 pd.DataFrame，这样它会将该列表的值作为列名。
    ...
    if not os.path.isdir(os.path.join(path_out, StudyInstance)):
        shutil.copytree(
            os.path.join(path_in, "C-"), os.path.join(path_out, StudyInstance)
        )
...
def model_ensemble(result1, result2, result3, result_out):
    ...
    rankavg_ICH = (
        PRED_ICH_cnn1 / np.max(PRED_ICH_cnn1 + smooth)
        + PRED_ICH_cnn2 / np.max(PRED_ICH_cnn2 + smooth)
        + PRED_ICH_cnn3 / np.max(PRED_ICH_cnn3 + smooth)
    ) / 3
    rankavg_EDH = (
        PRED_EDH_cnn1 / np.max(PRED_EDH_cnn1 + smooth)
        + PRED_EDH_cnn2 / np.max(PRED_EDH_cnn2 + smooth)
        + PRED_EDH_cnn3 / np.max(PRED_EDH_cnn3 + smooth)
    ) / 3
    rankavg_IPH = (
        PRED_IPH_cnn1 / np.max(PRED_IPH_cnn1 + smooth)
        + PRED_IPH_cnn2 / np.max(PRED_IPH_cnn2 + smooth)
        + PRED_IPH_cnn3 / np.max(PRED_IPH_cnn3 + smooth)
    ) / 3
    rankavg_IVH = (
        PRED_IVH_cnn1 / np.max(PRED_IVH_cnn1 + smooth)
        + PRED_IVH_cnn2 / np.max(PRED_IVH_cnn2 + smooth)
        + PRED_IVH_cnn3 / np.max(PRED_IVH_cnn3 + smooth)
    ) / 3
    rankavg_SAH = (
        PRED_SAH_cnn1 / np.max(PRED_SAH_cnn1 + smooth)
        + PRED_SAH_cnn2 / np.max(PRED_SAH_cnn2 + smooth)
        + PRED_SAH_cnn3 / np.max(PRED_SAH_cnn3 + smooth)
    ) / 3
    rankavg_SDH = (
        PRED_SDH_cnn1 / np.max(PRED_SDH_cnn1 + smooth)
        + PRED_SDH_cnn2 / np.max(PRED_SDH_cnn2 + smooth)
        + PRED_SDH_cnn3 / np.max(PRED_SDH_cnn3 + smooth)
    ) / 3
    ...
    # 存出csv檔
    RankAvg_df = pd.DataFrame(
        RankAvg[1:], columns=RankAvg[0]
    )  # 我们将 data[0] 作为 columns 参数传递给 pd.DataFrame，这样它会将该列表的值作为列名。
