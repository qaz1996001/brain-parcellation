...
def pipeline_aneurysm(
    ...
    try:
        ...
        if gpumRate < 0.6:
            ...
            shutil.copy(
                MRA_BRAIN_file, os.path.join(path_processID, "MRA_BRAIN.nii.gz")
            )
            ...
            # 複製nii到nii資料夾
            shutil.copy(
                os.path.join(path_processID, "MRA_BRAIN.nii.gz"),
                os.path.join(path_nii, "MRA_BRAIN.nii.gz"),
            )
            shutil.copy(
                os.path.join(path_processID, "Pred.nii.gz"),
                os.path.join(path_nii, "Pred.nii.gz"),
            )
            shutil.copy(
                os.path.join(path_processID, "Vessel.nii.gz"),
            ...
            if not os.path.isdir(
                os.path.join(path_dcm, "MRA_BRAIN")
            ):  # 如果資料夾不存在就建立
            ...
            calculate_aneurysm_long_axis_make_pred(
                path_dcm, path_processID, path_excel, ID
            )
            ...
            shutil.copy(
                os.path.join(path_processID, "Pred.nii.gz"),
                os.path.join(path_output, "Pred_Aneurysm.nii.gz"),
            )
            shutil.copy(
                os.path.join(path_processID, "Prob.nii.gz"),
                os.path.join(path_output, "Prob_Aneurysm.nii.gz"),
            )
            shutil.copy(
                os.path.join(path_processID, "Vessel.nii.gz"),
                os.path.join(path_output, "Pred_Aneurysm_Vessel.nii.gz"),
            )
            shutil.copy(
                os.path.join(path_processID, "Vessel_16.nii.gz"),
                os.path.join(path_output, "Pred_Aneurysm_Vessel16.nii.gz"),
            )
            shutil.copy(
                os.path.join(path_json_out, ID + "_platform_json.json"),
                os.path.join(path_output, "Pred_Aneurysm.json"),
            )
            shutil.copy(
                os.path.join(path_json_out, ID + "_platform_json.json"),
                os.path.join(path_output, "Pred_Aneurysm_platform_json.json"),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ID",
        type=str,
        default="1C95C88E_20171122_MR_1C95C88E",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "/mnt/e/pipeline/chuan/example_input/1C95C88E_20171122_MR_1C95C88E/MRA_BRAIN.nii.gz"
        ],
        help="用於輸入的檔案",
    )
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "/mnt/e/pipeline/chuan/example_inputDicom/1C95C88E_20171122_MR_1C95C88E/MRA_BRAIN/"
        ],
        help="用於輸入的檔案",
    )
    parser.add_argument(
        "--Output_folder",
        type=str,
        default="/mnt/e/pipeline/chuan/example_output/",
        help="用於輸出結果的資料夾",
    )
