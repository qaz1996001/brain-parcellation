...
def reslice_nifti_pred_nobrain(path_nii, path_reslice):
    ...
    new_img_nii = nibabel.processing.conform(img_nii, ((x_i, y_i, new_y_i)),
                                             (pixdim_img[1], pixdim_img[2], pixdim_img[1]), order=1)  #影像用
    new_pred_nii = nibabel.processing.conform(pred_nii, ((x_i, y_i, new_y_i)),
                                              (pixdim_img[1], pixdim_img[2], pixdim_img[1]), order=0)  #影像用
    new_vessel_nii = nibabel.processing.conform(vessel_nii, ((x_i, y_i, new_y_i)),
                                                (pixdim_img[1], pixdim_img[2], pixdim_img[1]), order=0)  #影像用
...
def create_MIP_pred(path_dcm, path_nii, path_png, gpu_num):
    ...
    for series in Series:
        ...
        for i in angle_list:
            ...
            if axis == 0:
                rotated_vessel = rotation_3d(translated_vessel, axis, -float(i), expand=True, label=True,
                                             gpu=gpu_num)  #rotated_vessel大小會一直變動
            elif axis == 1:
                rotated_vessel = rotation_3d(translated_vessel, axis, -float(i), expand=True, label=True,
                                             gpu=gpu_num)  #rotated_vessel大小會一直變動
            ...
            for j in range(pred_num):
                ...
                if axis == 0:
                    rotated_pred_one = rotation_3d(pred_one, axis, -float(i), expand=True, label=True,
                                                   gpu=gpu_num)  #rotated_vessel大小會一直變動
                elif axis == 1:
                    rotated_pred_one = rotation_3d(pred_one, axis, -float(i), expand=True, label=True,
                                                   gpu=gpu_num)  #rotated_vessel大小會一直變動
        ...
        new_pred = new_pred.cpu().numpy().astype(np.int16)  # optional: move back to CPU after processing
        ...
        new_dcm_seg.save_as(os.path.join(path_vesselMIP, series + '_' + str(0).rjust(4,
                                                                                     '0') + '_.dcm'))  #存出新dicom，同存同一層，因為有壓時間，所以不會互相覆蓋

        #因為做出新標註需要影像的affine matrix，所以這邊先做出dicom並轉出nifti
        for k in range(int(MIP_images.shape[-1])):
            dcm_slice = pydicom.dcmread(dcms_tofmra[-1])  #dicom每次都會連著修改一下，所以範本要在迴圈中讀取
            new_dcm_seg = img_to_MIPdicom(dcm_slice, MIP_images[:, :, k], series, series_uid, SE, k,
                                          k + 1)  #最後一項是image position所以多下降一次
            new_dcm_seg.save_as(os.path.join(path_vesselMIP, series + '_' + str(k).rjust(4,
                                                                                         '0') + '.dcm'))  #存出新dicom，同存同一層，因為有壓時間，所以不會互相覆蓋
        ...
        for l in range(int(np.max(new_pred))):
            ...
            areas = list(np.sum(predone[:, :, cover_z],
                                axis=(0, 1)))  #第二條件，如果相同遮擋率則算最大面積(例如都沒被遮中，遮擋率都是1)，即可知道哪張切片面積最大，cover_z這幾張比大小
...
def make_table_add_location(path_nii, path_excel):
    #讀取excel
    #讀取excel以獲得臨床資訊
    dtypes = {'PatientID': str, 'StudyDate': str, 'AccessionNumber': str, 'PatientName': str, 'Doctor': str,
              'birth_date': str,
    ...
    def make_dicomseg(mask, path_dcms, dcm_example, model_name, labels):
        ...
        if np.max(mask) > 0:
            ...
            dcm_seg_[0x10, 0x0010].value = dcm_one[
                0x10, 0x0010].value  #(0010,0010) Patient's Name <= (0010,0020) Patient ID
...
def compress_dicom_into_jpeglossless(path_dcm_in, path_dcm_out):
    ...
    for jdx, series in enumerate(Series):
        ...
        for dcm in dcms:
            ...
            SOPUID = dcm_slice[
                0x08, 0x0018].value  #(0008,0018) SOP Instance UID [1.2.840.113619.2.260.6945.3202356.28278.1273105263.493]
...
def make_pred_json(excel_file, path_dcm, path_nii, path_reslice, path_dicomseg, path_json, IDs, Series, group_id=49):
    ...
    #顏色這邊改成只有紅跟黃
    colorbar1 = ["red", "green", "cyan", "fuchsia", "orange", "lawngreen", "blue", "purple", "salmon",
                 "medium spring green", "dodger blue", "chocolate"]  #只記錄顏色，因為要重新編號

    #讀取excel以獲得臨床資訊
    dtypes = {'PatientID': str, 'StudyDate': str, 'AccessionNumber': str, 'PatientName': str,
              'Doctor': str}  #年齡在這裡就弄成數字吧
    #excel_name = 'Aneurysm_Pred_list_Val_merged_report.xlsx'
            ...
            for jdx, series in enumerate(Series):
                if series == 'MRA_BRAIN':
                    ...
                    if os.path.isfile(os.path.join(path_nii, 'Pred.nii.gz')) and AneurysmNumber > 0:
                        ...
                        SOPs = [pydicom.dcmread(y, force=True)[0x08, 0x0018].value for y in
                        ...
                        for o in range(AneurysmNumber):
                            ...
                            dcm_seg = pydicom.dcmread(
                                os.path.join(path_dicomseg, ID + '_' + series + '_' + 'A' + str(int(o + 1)) + '.dcm'),
                                force=True)
...
def upload_json(path_json):
    ...
    IDs = [y.split('_')[0:1][0] + '_' + y.split('_')[1:2][0] + '_' + y.split('_')[2:3][0] + '_' + y.split('_')[3:4][0]
           for y in files]
    ...
    for idx, ID in enumerate(IDs):
            ...
            if series == 'MRA_BRAIN':
                ...
                if len(str(json_data['Aneurysm_Number'])) > 0:
                    if json_data['Aneurysm_Number'] > 0:
                        bash_line = 'cd /var/www/shh-pacs && php artisan import:mask ' + os.path.join(path_json,
                                                                                                      ID + '_' + series + '.json')
        ...
        bash_line = 'cd /var/www/shh-pacs && php artisan import:sorted-image-ids ' + os.path.join(path_json,
                                                                                                  ID + '_sort.json')
