...
def custom_normalize_1(volume, mask=None, new_min=0., new_max=0.5, min_percentile=0.5, max_percentile=99.5,
...
def get_brain_seg(image_arr, spacing, model0):
    ...
    def rescale_volume(volume, new_min=0., new_max=1., min_percentile=0.5, max_percentile=99.5,
...
def predict_vessel(image_arr, brain_mask,
                   ...
                   verbose=False):
    def sliding_window_inference1(inputs, roi_size, model, overlap, n_class, importance_map, strategy="overlap_inside",
                                  mask=None, **kwargs):
...
def get_vessel_skeleton_labels(vessel_mask, spacing):
    ...
    for center in np.stack(np.where(skeleton_sr), axis=-1):
        ...
        z0 = int(center[2] - (d // 2))
        try:
            pseudo_vessel_sr[x0:x0 + d, y0:y0 + d, z0:z0 + d] = pseudo_vessel_sr[x0:x0 + d, y0:y0 + d,
                                                                z0:z0 + d] | ball(r)
        except:
            pass
    # diff vessel
...
def predict_aneurysm(image_arr, brain_mask, vessel_16labels, spacing,
                     ...
                     conf_th=0.1, min_diameter=1, top_k=4, obj_th=0.52, verbose=False):
    # resampling
    ...
    def sliding_window_inference2(inputs, roi_size, model, overlap, n_class, importance_map, strategy="overlap_inside",
                                  mask=None, **kwargs):
        image_size = tuple(inputs.shape[1:-1])
        roi_size = tuple(roi_size)
        # Padding to make sure that the image size is at least roi size
...
def save_nii(path_process, brain_mask, pred_vessel, vessel_16labels, pred_label, pred_prob_map,
             out_dir='test_label_out', verbose=False):
    ## load original image
...
def model_predict_aneurysm(path_code, path_process, case_name, path_log, gpu_n):
    ...
    try:
        ...
        if gpumRate < 0.6:
            ...
            bet_brain_mask[:, :, brain_bottom_idx:] = get_BET_brain_mask(image_arr[:, :, brain_bottom_idx:], spacing,
                                                                         bet_iter=1000)
            ...
            vessel_mask = combine_two_vessels(vessel_threshold, pred_vessel_mask, seed_mask=vessel_seed,
            ...
            # 5 Pred aneurysm
            pred_prob_map, df_pred, pred_label = predict_aneurysm(image_arr, brain_mask, vessel_mask, spacing, model2,
                                                                  verbose=verbose)
            ...
            save_nii(path_process, brain_mask, vessel_mask, vessel_16labels, pred_label, pred_prob_map,
