...
def custom_normalize_1(
    ...
    if robust_min != robust_max:
        new_volume = new_min + (new_volume - robust_min) / (robust_max - robust_min) * (
            new_max - new_min
        )
...
def get_struc(diameter, spacing):  # diameter in mm
    ...
    structure = resize_volume(
        ball(diameter * 5), target_size=structure_size, dtype="bool"
    )
...
# resampling
def resize_volume(
    arr, spacing=None, target_spacing=None, target_size=None, order=3, dtype="float32"
):
    ...
    if (spacing is not None) and (target_spacing is not None):
        ...
        out_vol = ndi.zoom(
            arr,
            zoom=scale,
            output=np.zeros(target_size, dtype=dtype),
            order=order,
            mode="grid-mirror",
            prefilter=True,
            grid_mode=False,
    ...
    if mask_types > 1:
        ...
        for i in range(mask_types):
            for z in range(mask.shape[-1]):
                ...
                dist_field_outer = ndi.distance_transform_edt(
                    np.logical_not(mask_mul[i][:, :, z])
                )
                mask_dist[i][:, :, z] = dist_field_inner - dist_field_outer
            mask_dist[i] = ndi.zoom(
                mask_dist[i],
                factor,
                order=1,
                mode="grid-mirror",
        ...
        for i in range(mask_types):
            mask_dist[i] = np.where(mask_dist[i] > 0.0, 1, 0)
        ...
        mask_new = np.where(mask_dist > 0.0, 1, 0)
        return mask_new.astype(mask.dtype)
...
def get_brain_seg(image_arr, spacing, model0):
    def center_crop(volume, centroid=None, size=192):
        ...
        if (r < x0 < w - r - 1) and (r < y0 < h - r - 1) and (r < z0 < d - r - 1):
            return volume[
                x0 - r : x0 - r + size, y0 - r : y0 - r + size, z0 - r : z0 - r + size
            ]
        else:
    ...
    image_iso = resize_volume(
        image_arr, spacing, target_spacing=target_spacing, order=0, dtype="float32"
    )
    z_pad = 192 - image_iso.shape[2]
    # print("z_pad =", z_pad)
    if z_pad >= 0:  # image_iso slices <= 192
        image_iso_pad = np.pad(
            image_iso, ((0, 0), (0, 0), (z_pad, 0)), "minimum"
        )  # pad z
    ...
    # model prediction
    seg_iso_crop = model0.serve(
        rescale_volume(image_iso_crop)[np.newaxis, ::-1, ::-1, :, np.newaxis]
    )
    synthseg33_array = np.argmax(seg_iso_crop[0, ::-1, ::-1, :, :], axis=3).astype(
        "uint8"
    )
    ...
    # seg postprocess
    if z_pad >= 0:  # image_iso slices <= 192
        seg_iso = center_pad(synthseg33_array, size=image_iso.shape)[
            :, :, z_pad:
        ]  # unpad z
    else:  # image_iso slices > 192
...
def get_BET_brain_mask(image_arr, spacing, bet_iter=1000, pad=32):
    ...
    image_iso = resize_volume(
        image_arr, spacing, target_spacing=target_spacing, order=0, dtype="int16"
    )

    pv = np.percentile(image_iso.ravel(), 15)
    print("pad value =", pv)
    image_iso = np.pad(
        image_iso, ((pad, pad), (pad, pad), (pad, pad)), "constant", constant_values=pv
    )  # minimum, mean

    # nibabel obj
    affine = np.array(
        [
            [target_spacing[0], 0, 0, 0],
    ...
    nib_image = nib.Nifti1Image(
        image_iso[::-1, ::-1, :], affine=affine
    )  # LPS to RAS orientation
    ...
    brain_mask = resize_volume(
        brain_mask_iso, target_size=image_arr.shape, dtype="bool"
    )
    return brain_mask
...
def modify_brain_mask(brain_mask, spacing, verbose=False):
    ...
    brain_mask = binary_erosion(
        brain_mask, get_struc(14, spacing).sum(axis=2, keepdims=True).astype(bool)
    )  # 12
    if verbose:
        print(".", end="")
...
class VesselSegmenter(object):
    ...
    def keep_brain_img(self, img, mask, spacing, radius_mm=40):
        ...
        disk_mask = disk(r)
        seed_area[x0 : x0 + disk_mask.shape[0], y0 : y0 + disk_mask.shape[1]] = (
            disk_mask
        )
    ...
    def distribution(
        self, x: np.ndarray, mu: float, sigma: float, stats: str, coefficient: float = 1
    ) -> np.ndarray:
    ...
    def threshold_segmentation(self, img: np.ndarray, mask, spacing) -> np.ndarray:
        ...
        p_normal = self.distribution(
            x, mu, sigma, stats="normal", coefficient=95
        )  # W 25
...
def get_vessel_seed(candidates, mask=None, spacing=(1, 1, 1)):
    ...
    # make major seeds
    major_seed_mask = binary_erosion(
        candidates_mask.copy(), get_struc(2.5, spacing)
    )  # 2.5, 3.0
...
def predict_vessel(
    ...
    def sliding_window_inference1(
        ...
        **kwargs,
        ...
        window_slices = get_window_slices(
            padded_image_size, roi_size, overlap, strategy
        )
        ...
        for i, window_slice in enumerate(window_slices):
            if (mask is None) or (
                ...
                and (
                    tf.math.reduce_any(
                        tf.slice(
                            mask_padded, begin=window_slice[0], size=window_slice[1]
                        )
                    )
                )
            ):
                window = tf.slice(
                    input_padded, begin=window_slice[0], size=window_slice[1]
                )
    ...
    if verbose:
        print(">>> predict_vessel ", end="")
    image_arr = custom_normalize_1(image_arr)

    importance_kernel = get_importance_kernel(
        patch_size, blend_mode="gaussian", sigma=sigma
    )
    importance_map = tf.tile(
        tf.reshape(importance_kernel, shape=[1, *patch_size, 1]),
        multiples=[1, 1, 1, 1, n_class],
    )
    # sliding_window_inference
    vessel_conf = sliding_window_inference1(
        inputs=image_arr[np.newaxis, :, :, :, np.newaxis],
        roi_size=patch_size,
        model=model1,
        overlap=overlap,
        n_class=n_class,
    ...
    if verbose:
        print(">>>", vessel_conf.shape)
    return vessel_conf > conf_th


# @title combine two vessel results
def combine_two_vessels(
    vessel_threshold, vessel_frangi, seed_mask=None, brain_mask=None
):
...
def get_vessel_skeleton_labels(vessel_mask, spacing):
    ...
    for center in np.stack(np.where(skeleton_sr), axis=-1):
        ...
        z0 = int(center[2] - (d // 2))
        try:
            pseudo_vessel_sr[x0 : x0 + d, y0 : y0 + d, z0 : z0 + d] = pseudo_vessel_sr[
                x0 : x0 + d, y0 : y0 + d, z0 : z0 + d
            ] | ball(r)
        except:
            pass
    # diff vessel
    ...
    pseudo_vessel = resize_volume(
        pseudo_vessel_sr, target_size=vessel_mask.shape, dtype="uint8"
    )
    diameter = (
        resize_volume(distance_sr, target_size=vessel_mask.shape, dtype="float32")
        * target_thickness
        * 2
    )
    skeleton = skeletonize(pseudo_vessel > 0) > 0
    diff_vessel = resize_volume(
        diff_vessel_sr, target_size=vessel_mask.shape, dtype="uint8"
    )
    # return diameter * skeleton.astype('float32'), diff_vessel

    # merge labels
    skeleton_labels = np.clip(
        expand_labels(skeleton.astype("uint8"), 1) + (diff_vessel * 2), 0, 2
    )
    return skeleton_labels


# @title predict vessel_16labels (w/ isotropic 0.8mm vol)
def predict_vessel_16labels(
    vessel_mask, model3, spacing, post_proc=True, min_size=3, verbose=False
):
    ...
    def crop_resample(arr, spacing, target_shape=(160, 160, 160)):
        arr = resize_volume(
            arr, spacing=spacing, target_spacing=(0.8, 0.8, 0.8), dtype="uint8"
        )
        zd = np.ceil(max(0, target_shape[2] - arr.shape[2])).astype("int32")
        arr = np.pad(arr, ((0, 0), (0, 0), (zd, 0)), "constant")
        # xy center crop
        x0, y0 = (
            np.floor(np.array(arr.shape[:2]) / 2).astype("int32")
            - np.array(target_shape[:2]) // 2
        )
        arr = arr[
            x0 : x0 + target_shape[0], y0 : y0 + target_shape[1], -target_shape[2] :
        ]
        return arr

    def back_to_origin(arr, spacing, target_shape=vessel_mask.shape):
        arr = resize_volume(
            arr, spacing=(0.8, 0.8, 0.8), target_spacing=spacing, dtype="uint8"
        )
    ...
    # run
    if verbose:
        print(">>> predict_vessel_16labels ", end="")
    vessel_mask_crop = crop_resample(vessel_mask, spacing).astype("float32")
    vessel_16labels_crop = model3.serve(
        vessel_mask_crop[np.newaxis, :, :, :, np.newaxis]
    ).numpy()[0]
...
def predict_aneurysm(
    ...
    image_iso = resize_volume(
        image_arr, spacing=spacing, target_spacing=target_spacing, dtype="float32"
    )
    vessel_iso = resize_volume(
        vessel_16labels, target_size=image_iso.shape, dtype="bool"
    )
    ...
    # sliding_window_inference
    pred_prob_map = sliding_window_inference2(
    ...
    # back to original spacing
    pred_prob_map = resize_volume(
        pred_prob_map, target_size=image_arr.shape, dtype="float32"
    )
    ...
    df_pred = pd.DataFrame(
        {
            ...
            "Pred_diameter": (
                (props["bbox-3"] - props["bbox-0"]) * spacing[0]
                + (props["bbox-4"] - props["bbox-1"]) * spacing[1]
            )
            / 2,
    ...
    for idx in df_pred.index:
        masked_veseel = vessel_16labels[
            new_pred_label == df_pred.loc[idx, "Pred_label"]
        ]
...
def save_nii(
    ...
    if verbose:
        ...
        print(
            f"shape={shape} spacing={spacing}  {image_arr.dtype}:{image_arr.min()}-{image_arr.max()}"
        )
        print("affine matrix =\n", aff)
    ...
    if pred_vessel.shape != original_size:
        pred_vessel = resize_volume(
            pred_vessel, target_size=original_size, dtype="uint8"
        )
    if vessel_16labels.shape != original_size:
        vessel_16labels = resize_volume(
            vessel_16labels, target_size=original_size, dtype="uint8"
        )
    if pred_label is not None:
        if pred_label.shape != original_size:
            pred_label = resize_volume(
                pred_label, target_size=original_size, dtype="uint8"
            )
    if pred_prob_map is not None:
        if pred_prob_map.shape != original_size:
            pred_prob_map = resize_volume(
                pred_prob_map, target_size=original_size, dtype="uint8"
            )
    ...
    vessel_16labels = vessel_16labels[::-1, ::-1, :].astype(
        "uint8"
    )  # LPS to RAS orientation
    ...
    try:
        ...
        if gpumRate < 0.6:
            ...
            model_root = os.path.join(
                path_model, "dataV2-brain_vessel-PosSample-ResUnet"
            )
            ...
            model_name = (
                "MP-FEMHv3_SHHv3_P3_iso-b8-FCresunet32-px_size_sw-Adam1E4_ema"  # iso
            )
            ...
            bet_brain_mask[:, :, brain_bottom_idx:] = get_BET_brain_mask(
                image_arr[:, :, brain_bottom_idx:], spacing, bet_iter=1000
            )
            brain_mask = modify_brain_mask(
                (brain_seg > 0) | (bet_brain_mask), spacing, verbose=verbose
            )
            del brain_seg, bet_brain_mask

            # 3 Get vessel mask
            vessel_threshold, _ = VesselSegmenter().threshold_segmentation(
                image_arr, brain_mask, spacing
            )
            vessel_seed = get_vessel_seed(
                vessel_threshold, mask=brain_mask, spacing=spacing
            )
            pred_vessel_mask = predict_vessel(
                image_arr, brain_mask, model1, verbose=verbose
            )
            vessel_mask = combine_two_vessels(
                vessel_threshold,
                pred_vessel_mask,
                seed_mask=vessel_seed,
            ...
            vessel_16labels = predict_vessel_16labels(
                vessel_mask, model3, spacing, verbose=verbose
            )

            # 5 Pred aneurysm
            pred_prob_map, df_pred, pred_label = predict_aneurysm(
                image_arr, brain_mask, vessel_mask, spacing, model2, verbose=verbose
            )
