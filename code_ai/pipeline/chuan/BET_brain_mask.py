...
def resize_volume(
    arr, spacing=None, target_spacing=None, target_size=None, dtype="float32"
):
    ori_size = arr.shape
...
def get_BET_brain_mask(image_arr, spacing, bet_iter=1000):
    ...
    image_iso = resize_volume(
        image_arr, spacing, target_spacing=target_spacing, dtype="float32"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--SpacingBetweenSlices", type=float, help="SpacingBetweenSlices"
    )
    ...
    spacing = [
        float(args.PixelSpacing),
        float(args.PixelSpacing),
        float(args.SpacingBetweenSlices),
    ]
