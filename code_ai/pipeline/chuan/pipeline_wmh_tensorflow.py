...
def pipeline_wmh(
    ...
    try:
        ...
        shutil.copy(
            SynthSEG_WM_file, os.path.join(path_nii, ID + "_SynthSEG_WM.nii.gz")
        )
        ...
        T2FLAIR_nii = nib.load(
            os.path.join(path_nii, ID + "_T2FLAIR.nii.gz")
        )  # 讀取影像
        ...
        T2FLAIR_array = np.load(
            os.path.join(path_tsnpy, ID + "_T2FLAIR.npy")
        )  # 讀出image的array矩陣，用nii讀影像的話會有背景問題
        SynthSEG_WM_nii = nib.load(
            os.path.join(path_nii, ID + "_SynthSEG_WM.nii.gz")
        ...
        new_y_pred_synthseg = data_translate_back(Y_SynthSEG_WMH, T2FLAIR_nii).astype(
            int
        )
        ...
        nib.save(
            new_y_pred_synthseg_nii,
            os.path.join(path_output, "Pred_WMH_synthseg.nii.gz"),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ID",
        type=str,
        default="00003092_20201007_MR_20910070157",
