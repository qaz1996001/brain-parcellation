...
def get_diameter(
    ...
    try:
        ...
        return (
            (
                (props["bbox-2"] - props["bbox-0"]) * spacing[0]
                + (props["bbox-3"] - props["bbox-1"]) * spacing[1]
            )
            / 2.0
        ).item()
    except:
...
def make_mask_json(
    ...
    for index, referenced_series in enumerate(referenced_series_list):
        ...
        for jj, instance in enumerate(instance_list):
            ...
            filtered_items = next(
                filter(
                    lambda x: x[1].get((0x0008, 0x0018)).value == sop_instance_uid,
            ...
            prob_max = "1"
            is_ai = "1"
            series_type = os.path.basename(
                os.path.dirname(sorted_dcms[filtered_items[0]])
            )
...
def make_study_json(
    source_images: List[Union[FileDataset, DicomDir]], group_id: str = GROUP_ID
) -> AITeamRequest:
    instance_list = []
    series_list = []
    at_team_dict = dict()
    ...
    for index, dicom_ds in enumerate(source_images):
        ...
        instance_dict.update(
            dict(
                sop_instance_uid=sop_instance_uid, projection=str(image_osition_patient)
            )
        )
        ...
        if index == 0:
            series_dict.update(
                dict(series_instance_uid=series_instance_uid, instance=instance_list)
            )
            series_list.append(series_dict)
            sorted_dict.update(
                dict(study_instance_uid=study_instance_uid, series=series_list)
            )
            study_dict.update(
                dict(
                    group_id=group_id,
            ...
            at_team_dict.update(dict(study=study_dict, sorted=sorted_dict, mask=None))
...
def process_prediction_mask(
    pred_data: np.ndarray, path_dcms: str, series_name: str, output_folder: pathlib.Path
    ...
    for index, i in enumerate(pred_data_unique):
        ...
        if np.sum(mask) > 0:
            ...
            temp_mask_request = make_mask_json(
    ...
    mask_request = MaskRequest(
        study_instance_uid=first_mask_request.study_instance_uid,
...
def main():
    ...
    # 處理預測遮罩
    at_team_request: AITeamRequest = process_prediction_mask(
        new_nifti_array, str(path_dcms), series, series_folder
    )
    platform_json_path = series_folder.joinpath(
        path_nii.name.replace(".nii.gz", "_platform_json.json")
    )
    print("platform_json_path", platform_json_path)
    at_team_request.mask.instances = sorted(
        at_team_request.mask.instances, key=lambda instance: instance.mask_index
    )
