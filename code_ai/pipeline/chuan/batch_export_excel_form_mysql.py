...
def export_excel_from_mysql(path_in, path_out, models):
    ...
    excel_export = (
        "/var/www/tmu-pacs/laravel/storage/app/study.xlsx"  # php程式輸出的excel
    )
    ...
    if "StudyDate" in df.columns:
        ...
        if len(SDATEs) > 0:
            ...
            if len(str(SDATEs[0])) == 12:
                ...
                bg_request_date = [
                    str(y).rjust(8, "0") + "0000" for y in SDATEs
                ]  # 加上0點0分
                sm_request_date = [
                    str(y).rjust(8, "0") + "2359" for y in SDATEs
                ]  # 加上23點59分
            else:
                bg_request_date = ""
                sm_request_date = ""
    ...
    if len(PIDs) > 0 and len(bg_request_date) > 0 and len(ANs) > 0:
        for pid, brd, srd, an in zip(PIDs, bg_request_date, sm_request_date, ANs):
            ...
            if len(pid + brd) == 20 or len(an) > 0:
                for model in models:
                    bash_line = (
                        'cd /var/www/tmu-pacs/laravel && php artisan export:predict "'
                        + start_time
                        + '" "'
                        + end_time
                        + '" --type='
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i",
        type=str,
        default="/home/tmu/Desktop/export_excel_example/example1.xlsx",
        help="撈取MySQL資料要輸入的Excel檔案路徑，欄位必須有PatientID或是AccessionNumber，加上StudyDate(可選)",
    )  # 在同一層輸出結果
    parser.add_argument(
        "-o",
        type=str,
        default="/home/tmu/Desktop/study.xlsx",
        help="撈取MySQL資料要輸出的Excel檔案路徑",
    )  # 如果沒有標註檔，常態維持None
    parser.add_argument(
        "-model",
        nargs="+",
        type=str,
        default=["WMH", "STROKE", "CVR"],
        help="要查詢哪些模型的結果，要以list方式輸入，例如: WMH STROKE",
    )
