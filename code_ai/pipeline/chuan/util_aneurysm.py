...
from .create_dicomseg_multi_file_json_claude import (
...
def reslice_nifti_pred_nobrain(path_nii, path_reslice):
    ...
    new_img_nii = nibabel.processing.conform(
        img_nii,
        ((x_i, y_i, new_y_i)),
        (pixdim_img[1], pixdim_img[2], pixdim_img[1]),
        order=1,
    )  # 影像用
...
def create_MIP_pred(path_dcm, path_nii, path_png, gpu_num):
    ...
    def img_to_MIPdicom(dcm, img, tag, series, SE, angle, count):
        ...
        # 計算新的位置
        slice_thickness = PixelSpacing[
            0
        ]  # 假設使用 PixelSpacing 的第一個值來定義每一層的厚度
        new_position = np.array(ImagePosition) - normal_vector * slice_thickness * count

        dcm[0x20, 0x0032].value = [
    ...
    def dilation3d(x):
        ...
        x_di = ndimage.binary_dilation(
            x, structure=kernel, iterations=15
        )  # 亂做n次，kernel越小算越快
        return x_di
    ...
    def rotation_3d(X, axis, theta, expand=True, fill=0.0, label=False, gpu=None):
        ...
        if axis == 0:
            interpolation_mode = (
                InterpolationMode.NEAREST if label else InterpolationMode.BILINEAR
            )
            X = rotate_torch(
                X,
    ...
    class createMIP:
        ...
        def process_images(
            ...
            for i in angle_list:
                if axis == 0:
                    output1 = self.rotation_3d(
                        translated_img, 0, -float(i), expand=True, label=label
                    )  # 水平
                elif axis == 1:
                    output1 = self.rotation_3d(
                        translated_img, 1, -float(i), expand=True, label=label
                    )  # 垂直旋轉
    ...
    vessel_z_list = [
        y for y, z in enumerate(vessel_z) if z > 0
    ]  # 做前面那點label是在哪裡
    ...
    for series in Series:
        ...
        print(
            "Time taken for {} sec\n".format(time.time() - start_img),
            " MIP_images is OK!!!, Next...",
        )

        # 血管確認是否有阻擋是改成前後一起看，所以可以當作MIP後確認重疊比例，所以可以label也做mip、vessel也做mip
        # MIP血管還是要做，因為要產生血管的mask
        MIP_vessels = createrMIP.process_images(
            translated_vessel,
            angle,
            angle_step,
            y_i,
        ...
        cover_ranges_pred = np.zeros(
            (int(MIP_images.shape[-1]), int(np.max(pred)))
        )  # 記錄哪張血管遮擋最少

        angle_list = np.arange(0, angle, angle_step)
        # new_pred = np.zeros((y_i, x_i, angle_list.shape[0])) #會跟mip的影像一樣大，雖已知大小，但跟for迴圈有關，跟旋轉的角度一樣多
        # Rewritten version for CUDA computation
        new_pred = torch.zeros(
            (y_i, x_i, angle_list.shape[0]), dtype=torch.int16, device="cuda"
        )

        for i in angle_list:
            # 結果還是要做出血管個別旋轉的圖
            if axis == 0:
                rotated_vessel = rotation_3d(
                    translated_vessel,
                    axis,
                    -float(i),
                    expand=True,
            ...
            for j in range(pred_num):
                ...
                if axis == 0:
                    rotated_pred_one = rotation_3d(
                        pred_one, axis, -float(i), expand=True, label=True, gpu=gpu_num
                    )  # rotated_vessel大小會一直變動
                elif axis == 1:
                    rotated_pred_one = rotation_3d(
                        pred_one, axis, -float(i), expand=True, label=True, gpu=gpu_num
                ...
                pred_z_list = torch.nonzero(
                    pred_z > 0, as_tuple=False
                ).squeeze()  # 做前面那點label是在哪裡
                ...
                if axis == 0:
                    if x_r > x_i:
                        slice_y = (y_i - y_r) // 2
                        slice_x = (x_r - x_i) // 2
                        extra_pred[slice_y : slice_y + y_r, :] = pred_mip[
                            :, slice_x : slice_x + x_i
                        ]
                    else:
                        slice_y = (y_i - y_r) // 2
                        slice_x = (x_i - x_r) // 2
                        extra_pred[slice_y : slice_y + y_r, slice_x : slice_x + x_r] = (
                            pred_mip
                        )
                else:
                    if y_r > y_i:
                        ...
                        slice_x = (x_r - x_i) // 2
                        extra_pred = pred_mip[
                            slice_y : slice_y + y_i, slice_x : slice_x + x_i
                        ]
                    else:
                        slice_y = (y_i - y_r) // 2
                        slice_x = (x_i - x_r) // 2
                ...
                if torch.sum(extra_pred) > 0:
                    cover_range = (
                        torch.sum(pred_cover).float() / torch.sum(pred_mip_s).float()
                    )
        ...
        new_pred = (
            new_pred.cpu().numpy().astype(np.int16)
        )  # optional: move back to CPU after processing
        ...
        fig_pitch[
            y_displacement : pitch_resize.shape[0] + y_displacement,
            x_displacement : pitch_resize.shape[1] + x_displacement,
        ] = pitch_resize
        fig_pitch[fig_pitch > 0] = np.max(
            MIP_images[:, :, 0]
        )  # 因為疊上去讓他報數值，這樣背景就會變暗
        output_pitch = cv2.add(MIP_mark, fig_pitch)  # 疊加第一張
        dcm_slice = pydicom.dcmread(
            dcms_tofmra[-1]
        )  # dicom每次都會連著修改一下，所以範本要在迴圈中讀取
        new_dcm_seg = img_to_MIPdicom(
            dcm_slice, output_pitch, series, series_uid, SE, "00", 0
        )  # 從0開始
        new_dcm_seg.save_as(
            os.path.join(path_vesselMIP, series + "_" + str(0).rjust(4, "0") + "_.dcm")
        )  # 存出新dicom，同存同一層，因為有壓時間，所以不會互相覆蓋
        ...
        bash_line = (
            "dcm2niix -z y -f " + series + " -o " + path_nii + " " + path_vesselMIP
        )  # 把tag檔案複製，-v : verbose (n/y or 0/1/2, default 0) [no, yes, logorrheic]
        # print('bash_line:', bash_line)
        os.system(bash_line)

        # 讀取nifti取得新affine matrix
        img_pitch_nii = nib.load(
            os.path.join(path_nii, series + ".nii.gz")
        )  # ADC要讀因為會取ADC值判斷
        ...
        for l in range(int(np.max(new_pred))):
            ...
            cover_z = np.where(cover_list == np.max(cover_list))[
                0
            ]  # 第一個條件 => 遮擋率最低
            predone = (new_pred == l + 1).astype(int).copy()
            areas = list(
                np.sum(predone[:, :, cover_z], axis=(0, 1))
            )  # 第二條件，如果相同遮擋率則算最大面積(例如都沒被遮中，遮擋率都是1)，即可知道哪張切片面積最大，cover_z這幾張比大小
            ...
            if index_area == 0:
                ...
                pred_area[:, :, index_area - 1 : index_area + 2] = predone[
                    :, :, index_area - 1 : index_area + 2
                ]
...
def dilation3d(x):
    kernel = np.ones((3, 3, 3), dtype=int)  # 3d dilation
    x_di = ndimage.binary_dilation(
        x, structure=kernel, iterations=1
    )  # 亂做n次，kernel越小算越快
    return x_di
...
def calculate_aneurysm_long_axis_make_pred(path_dcm, path_nii, path_excel, ID):
    ...
    df.to_excel(
        os.path.join(path_excel, "Aneurysm_Pred_long_axis_list.xlsx"), index=False
    )
    ...
    for i in range(int(np.max(mask_array))):
        ...
        if len(y_cl) > 0:
            ...
            cluster_matrix[y_cl, x_cl, z_cl] = (
                254  # cv2的白，因為cv2是在 [0-255] base的，所以=255
            )
            cluster_z_list = np.sum(cluster_matrix, axis=(0, 1))  # 對z軸sum
            z_cluster_l = np.where(cluster_z_list > 0)
            z_c_l_num = len(z_cluster_l[0])
            z_cluster_m = np.zeros(
                (z_c_l_num)
            )  # 做跟有aneurysm z軸等長的矩陣以方便np.max比較，真實長度
            z_cluster_int = np.zeros(
                (z_c_l_num)
            )  # 做跟有aneurysm z軸等長的矩陣以方便np.max比較，pixel長度
            site = []
            for j in range(z_c_l_num):
                mask_fig = cluster_matrix[
                    :, :, z_cluster_l[0][j]
                ]  # 抓特定平面出來，去計算
            ...
            # 製作成excel
            df.to_excel(
                os.path.join(path_excel, "Aneurysm_Pred_long_axis_list.xlsx"),
                index=False,
            )
...
def make_table_row_patient_pred(path_excel, ID):
    ...
    for idx, (PID1, SDate1, AN1) in enumerate(zip(PID1s, SDate1s, AN1s)):
        ...
        ane_list = [
            jdx for jdx, y in enumerate(ID2s) if y == PID1 + "_" + AN1
        ]  # 抓出相同病人的位置
        ...
        # 製作成excel
        mm_dict_pd.to_excel(
            os.path.join(path_excel, "Aneurysm_Pred_list.xlsx"), index=False
        )
...
def make_table_add_location(path_nii, path_excel):
    ...
    df = pd.read_excel(os.path.join(path_excel, excel_file), dtype=dtypes).fillna(
        ""
    )  # 將指定欄位改成特定格式
    ...
    for idx, (PID, Sdate, AN, PA) in enumerate(zip(PIDs, Sdates, ANs, PAs)):
        ...
        if PA != "00657109_20210413_MR_21004130157":
            ...
            labels16_nii = nib.load(
                os.path.join(path_nii, "Pred_Location16labels.nii.gz")
            )  # 這邊預設不同影像也是同標註
            ...
            for k in range(Label_num):
                ...
                assert num_unique_values_16labels == 1, print(
                    "挫屎!!! 同一顆怎麼那麼多location"
                )

                # 接下來是用判別式判斷填入哪個location跟sublocation值
                if (
                    int(np.unique(labels16_list)[0]) == 1
                    or int(np.unique(labels16_list)[0]) == 3
                ):
                    df.at[idx, str(k + 1) + "_Location"] = "ICA"
                    df.at[idx, str(k + 1) + "_SubLocation"] = ""
                elif (
                    int(np.unique(labels16_list)[0]) == 2
                    or int(np.unique(labels16_list)[0]) == 4
                ):
                    df.at[idx, str(k + 1) + "_Location"] = "MCA"
                    df.at[idx, str(k + 1) + "_SubLocation"] = ""
                elif (
                    int(np.unique(labels16_list)[0]) == 5
                    or int(np.unique(labels16_list)[0]) == 6
                ):
                    ...
                    int(np.unique(labels16_list)[0]) == 8
                    or int(np.unique(labels16_list)[0]) == 9
                ):
                    df.at[idx, str(k + 1) + "_Location"] = "ICA"
                    df.at[idx, str(k + 1) + "_SubLocation"] = "p"
                elif (
                    int(np.unique(labels16_list)[0]) == 10
                    or int(np.unique(labels16_list)[0]) == 11
                ):
                    df.at[idx, str(k + 1) + "_Location"] = "PCA"
                    df.at[idx, str(k + 1) + "_SubLocation"] = ""
                elif (
                    int(np.unique(labels16_list)[0]) == 12
                    or int(np.unique(labels16_list)[0]) == 13
                ):
                    ...
                    int(np.unique(labels16_list)[0]) == 15
                    or int(np.unique(labels16_list)[0]) == 16
                ):
...
def create_dicomseg_multi_file(
    path_code, path_img_dcm, path_nii, path_nii_resample, path_dcmseg, ID
):
    example_file = os.path.join(path_code, "example", "SEG_20230210_160056_635_S3.dcm")
    # 針對5類設計影像顏色
    model_name = "Aneurysm_AI"

    labels_vessel = {1: {"SegmentLabel": "Vessel", "color": "red"}}
    ...
    def make_dicomseg(mask, path_dcms, dcm_example, model_name, labels):
        ...
        if np.max(mask) > 0:
            ...
            source_images = [
                pydicom.dcmread(x, stop_before_pixels=True) for x in new_dcms
            ]
            dcm_seg_ = writer.write(segmentation, source_images)

            # 增加一些標註需要的資訊，作法，輸入範例資訊，再把實際case的值填入
            dcm_seg_[0x10, 0x0010].value = dcm_one[
                0x10, 0x0010
            ].value  # (0010,0010) Patient's Name <= (0010,0020) Patient ID
            dcm_seg_[0x20, 0x0011].value = dcm_one[
                0x20, 0x0011
            ].value  # (0020,0011) Series Number [3]
            ...
            dcm_seg_[0x5200, 0x9229][0][0x20, 0x9116][0][0x20, 0x0037].value = dcm_one[
                0x20, 0x0037
            ].value  # (0020,0037) Image Orientation (Patient)
            dcm_seg_[0x5200, 0x9229][0][0x28, 0x9110][0][0x18, 0x0050].value = dcm_one[
                0x18, 0x0050
            ].value  # (0018,0050) Slice Thickness
            dcm_seg_[0x5200, 0x9229][0][0x28, 0x9110][0][0x18, 0x0088].value = dcm_one[
                0x18, 0x0088
            ].value  # (0018,0088) Spacing Between Slices
            dcm_seg_[0x5200, 0x9229][0][0x28, 0x9110][0][0x28, 0x0030].value = dcm_one[
                0x28, 0x0030
            ].value  # (0028,0030) Pixel Spacing
    ...
    for idx, ID in enumerate(IDs):
        ...
        for idx_s, series in enumerate(Series):
            ...
            if series == "MRA_BRAIN":
                ...
                Pred_nii = nib.load(
                    os.path.join(path_nii_resample, series + "_pred.nii.gz")
                )
                Pred = np.array(
                    Pred_nii.dataobj
                )  # 讀出label的array矩陣      #256*256*22
                Vessel_nii = nib.load(
                    os.path.join(path_nii_resample, series + "_vessel.nii.gz")
                )
                Vessel = np.array(
                    Vessel_nii.dataobj
                )  # 讀出label的array矩陣      #256*256*22
            ...
            for i in range(int(np.max(Pred))):
                ...
                # 底下做成dicom-seg
                dcm_seg = make_dicomseg(
                    new_Pred.astype("uint8"),
                    path_dcms,
                    dcm_example,
                    model_name,
                    labels_ane,
...
def compress_dicom_into_jpeglossless(path_dcm_in, path_dcm_out):
    ...
    for jdx, series in enumerate(Series):
        ...
        for dcm in dcms:
            dcm_slice = pydicom.dcmread(
                os.path.join(path_dcm_in, series, dcm)
            )  # ADC的切片
            # (0002,0003) Media Stored SOP Instance UID  [1.2.840.113619.2.260.6945.3202356.28278.1273105263.493]
            SOPUID = dcm_slice[
                0x08, 0x0018
            ].value  # (0008,0018) SOP Instance UID [1.2.840.113619.2.260.6945.3202356.28278.1273105263.493]
    ...
    # 最後複製dicom-seg
    shutil.copytree(
        os.path.join(path_dcm_in, "Dicom-Seg"), os.path.join(path_dcm_out, "Dicom-Seg")
    )
        ...
        if os.path.isfile(os.path.join(path_nii, "Pred.nii.gz")):
            ...
            for jdx, series in enumerate(Series):
                if series == "MRA_BRAIN":
                    ...
                    resolution_y, resolution_x, resolution_z = (
                        pred.shape
                    )  # 這裡獲得影像大小
                    ...
                    reader = sitk.ImageSeriesReader()
                    files = sorted(
                        reader.GetGDCMSeriesFileNames(os.path.join(path_dcm, series))
                    )
                    # Stack the 2D slices to form a 3D array representing the volume
                    dcms = [
                        pydicom.dcmread(os.path.join(path_dcm, series, y))
                        for y in files
                    ]
                    ...
                    for dcm, file in zip(dcms, files):
                        ...
                        sort_instances += [
                            {"sop_instance_uid": SOPuid, "projection": projection}
                        ]
                    ...
                    sorted_dcms, image, first_dcm, source_images = (
                        load_and_sort_dicom_files(os.path.join(path_dcm, series))
                    )
                    ...
                    if (
                        os.path.isfile(os.path.join(path_nii, "Pred.nii.gz"))
                        and AneurysmNumber > 0
                    ):
                        pred_nii = nib.load(
                            os.path.join(path_nii, "Pred.nii.gz")
                        )  # 這邊預設不同影像也是同標註
                        ...
                        SOPs = [
                            pydicom.dcmread(y, force=True)[0x08, 0x0018].value
                            for y in New_dcms
                        ]  # (0008,0018) SOP Instance UID [1.2.840.113619.2.353.6945.3202356.14286.1484267644.90]

                        # 這邊要先取得標註的哪張是isMainSeg，採用單點最大機率，建立list紀錄單顆資訊
                        seg_dict = {}
                        seg_dict["series_instance_uid"] = ai_team_request.sorted.series[
                            0
                        ].series_instance_uid
                        ...
                        for o in range(AneurysmNumber):
                            ...
                            try:
                                SubLocation = str(
                                    int(df[str(o + 1) + "_SubLocation"][study_index])
                                )
                            except:
                                SubLocation = str(
                                    df[str(o + 1) + "_SubLocation"][study_index]
                                )
                            ...
                            segment_attribute = {
                                ...
                                "diameter": str(
                                    round(df[str(o + 1) + "_size"][study_index], 1)
                                ),
                                "type": "saccular",
                                "location": str(
                                    df[str(o + 1) + "_Location"][study_index]
                                ),
                                "sub_location": SubLocation,
                                "checked": "1",
                                "prob_max": str(
                                    round(df[str(o + 1) + "_Prob_max"][study_index], 2)
                                ),
                                "is_ai": "1",
                                "seg_series_instance_uid": dcm_seg[0x20, 0x000E].value,
                                "seg_sop_instance_uid": dcm_seg[0x08, 0x0018].value,
                                "dicom_sop_instance_uid": SOPs[
                                    int(np.median(have_labels))
                                ],
                    ...
                    pred_nii = nib.load(
                        os.path.join(path_reslice, series + "_pred.nii.gz")
                    )  # 這邊預設不同影像也是同標註
                    pred = np.array(pred_nii.dataobj)  # 讀出image的array矩陣
                    pred = data_translate(pred, pred_nii).astype("int16")
                    resolution_y, resolution_x, resolution_z = (
                    ...
                    ai_team_request.sorted.series.append(
                        ai_team_request_mip.sorted.series[0]
                    )
                    ...
                    if (
                        ...
                        # 這邊要先取得標註的哪張是isMainSeg，採用單點最大機率，建立list紀錄單顆資訊
                        seg_dict = {}
                        seg_dict["series_instance_uid"] = (
                            ai_team_request_mip.sorted.series[0].series_instance_uid
                        )
                        if series == "MIP_Pitch":
                            seg_dict["series_type"] = "2"
                        elif series == "MIP_Yaw":
...
def upload_json(path_json):
    ...
    for idx, ID in enumerate(IDs):
        ...
        for jdx, series in enumerate(Series):
            ...
            if series == "MRA_BRAIN":
                ...
                response = requests.post(
                    url, data=study_data, headers={"Accept": "application/json"}
                )
                ...
                if len(str(json_data["Aneurysm_Number"])) > 0:
                    if json_data["Aneurysm_Number"] > 0:
                        bash_line = (
                            "cd /var/www/shh-pacs && php artisan import:mask "
                            + os.path.join(path_json, ID + "_" + series + ".json")
                        )
        ...
        bash_line = (
            "cd /var/www/shh-pacs && php artisan import:sorted-image-ids "
