...
def model_predict_stroke(
    path_code, path_process, cuatom_model, case_name, path_log, gpu_n
):
    # 以log紀錄資訊，先建置log
    ...
    try:
        ...
        if gpumRate < 0.6:
            ...
            class ResBlock(Layer):
                ...
                def get_config(self):
                    return dict(
                        ...
                        **super(ResBlock, self).get_config(),
                    )

            def ResUNet(
                input_shape, classes: int, filters_root: int = 64, depth: int = 3
            ):
                """
                ...
                if not math.log(input_shape[0], 2).is_integer() or not math.log(
                    input_shape[1], 2
                ):
                    raise ValueError(
                        f"Input height ({input_shape[0]}) and width ({input_shape[1]}) must be power of two."
                    )
                if 2**depth > min(input_shape[0], input_shape[1]):
                    raise ValueError(
                        f"Model has insufficient height ({input_shape[0]}) and width ({input_shape[1]}) compared to its desired depth ({depth})."
                    )
                ...
                layer = Conv2D(
                    filters=classes, kernel_size=1, strides=1, padding="same"
                )(layer)
            ...
            model = ResUNet(
                input_shape=(256, 256, 2), classes=1, filters_root=32, depth=5
            )
            ...
            img_nii = nib.load(
                os.path.join(path_nii, case_name + "_DWI1000.nii.gz")
            )  # 讀取影像

            # 以上前處理完成，開始predict => .png
            ADC_array = np.load(
                os.path.join(path_npy, case_name + "_ADC.npy")
            )  # 讀出image的array矩陣，用nii讀影像的話會有背景問題   #256*256*22
            DWI1000_array = np.load(
                os.path.join(path_npy, case_name + "_DWI1000.npy")
            ...
            img_test[1, slice_y : slice_y + y_i, slice_x : slice_x + x_i, :] = (
                DWI1000_array
            )
            img_test = img_test.swapaxes(
                0, 3
            )  # 交换轴0和3，因為model_predict_batch 是 張數,y,x,c
            ...
            nib.save(
                Y_pred_nii, os.path.join(path_predict, case_name + "_prediction.nii.gz")
            )
...
if __name__ == "__main__":
    ...
    path_process = os.path.join(
        "/data/pipeline/chuan/process/Deep_Infarct", case_name
