...
def pipeline_wmh(
    ...
    try:
        ...
        Pred_WMH_synthseg = os.path.join(
            str(path_output_dir), "Pred_WMH_synthseg.nii.gz"
        )
...
if __name__ == "__main__":
    ...
    # 直接當作function的輸入
    Pred_WMH_synthseg, Pred_WMH, json_path_name2 = pipeline_wmh(
        ID,
        T2FLAIR_file,
        SynthSEG_WM_file,
        SynthSEG_file,
    ...
    if Pred_WMH_synthseg is not None:
        stdout, stderr = dicom_seg_multi_file(
            ID, InputsDicomDir, Pred_WMH_synthseg, path_output
        )
        upload_dicom_seg(
            path_output,
            Pred_WMH_synthseg,
    ...
    if SynthSEG_WM_file is not None:
        stdout, stderr = dicom_seg_multi_file(
            ID, InputsDicomDir, SynthSEG_WM_file, path_output
        )
