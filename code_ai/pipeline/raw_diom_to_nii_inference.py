...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_dicom", type=str, help="input_dicom  /mnt/e/raw_dicom "
    )
    parser.add_argument(
        "--output_dicom", type=str, help="output_dicom /mnt/e/rename_dicom_0407 "
    )
    parser.add_argument(
        "--output_nifti", type=str, help="output_nifti /mnt/e/rename_nifti_0407"
    )
    ...
    if len(result_list) > 0:
        ...
        output_nifti_path_list = list(
            map(lambda x: output_nifti_path.joinpath(os.path.basename(x)), result_list)
        )
        for nifti_study_path in output_nifti_path_list:
            ...
            if dicom_study_path.name in study_id_set:
                ...
                task_pipeline_result = task_pipeline_inference.push(
                    {
