...
class PlatformJSONBuilder(Generic[T], metaclass=abc.ABCMeta):
    ...
    def set_sorted(self, source_images: List[Union[FileDataset, DicomDir]]) -> Self:
        ...
        for index, dicom_ds in enumerate(source_images):
            ...
            if index == 0:
                ...
                series_dict.update(
                    dict(
                        series_instance_uid=series_instance_uid, instance=instance_list
                    )
                )
    ...
    def build(self) -> T:
        self._ai_team_dict.update(
            {
                "study": self._study_request if self._study_request else None,
...
class ReviewPlatformJSONBuilder(
    Generic[
        AITeamT,
        StudyT,
        StudySeriesT,
...
class ReviewBasePlatformJSONBuilder(
    ReviewPlatformJSONBuilder[
        AITeamRequest,
        StudyRequest,
        StudySeriesRequest,
        StudyModelRequest,
