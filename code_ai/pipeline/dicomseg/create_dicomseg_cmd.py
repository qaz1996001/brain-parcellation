...
def make_mask_series_json(source_images: List[Union[FileDataset, DicomDir]],
    ...
    mask_index     = filter_cmd['mask_index']
...
def make_mask_json(source_images: List[Union[FileDataset, DicomDir]],
    ...
    for index, reslut in enumerate(reslut_list):
        ...
        filter_cmd.update({'main_seg_slice':reslut['main_seg_slice'],
...
def main():
    ...
    platform_json_path = series_folder.joinpath(path_nii.name.replace('.nii.gz',
                                                                      '_platform_json.json'))
