...
from code_ai.pipeline.dicomseg.schema.cmb import (
    CMBMaskRequest,
    CMBMaskSeriesRequest,
    CMBMaskInstanceRequest,
)
from code_ai.pipeline.dicomseg.schema.cmb import (
    CMBAITeamRequest,
...
class CMBPlatformJSONBuilder(PlatformJSONBuilder[CMBAITeamRequest]):
        ...
        **kwargs,
        ...
        for index, reslut in enumerate(reslut_list):
            filter_cmd_data = list(
                filter(lambda x: x["label#"] == reslut["mask_index"], pred_json_list)
            )
            if filter_cmd_data:
                pass
            ...
            mask_instance_list.append(
                self.build_mask_instance(source_images, dcm_seg, *args, **kwargs)
            )
    ...
    def set_study(
        ...
        study_dict.update({"series": study_series, "model": study_model})
...
class NewReviewCMBPlatformJSONBuilder(ReviewCMBPlatformJSONBuilder):
    ...
    def build_mask(
        ...
        for index, (series_name, series_source_images) in enumerate(
            ...
            mask_model_series: CMBMaskModel2Request = self.get_mask_model(
...
def main_review_cmd():
    ...
    pred_json_path = path_nii.parent.joinpath(path_nii.name.replace(".nii.gz", ".json"))

    sorted_dcms, image, first_dcm, source_images = utils.load_and_sort_dicom_files(
        str(path_dcms)
    )
    ...
    pred_json_list = [
        {"series_type": SeriesTypeEnum.SWAN, "data": pred_json},
    ]
    # cmb_platform_json_builder = ReviewCMBPlatformJSONBuilder()
    cmb_platform_json_builder = NewReviewCMBPlatformJSONBuilder()
    cmb_platform_json = (
        cmb_platform_json_builder.set_series_type(
            SeriesTypeEnum.SWAN, source_images=source_images
        )
        .set_group_id(group_id)
        .build_sorted()
        .build_study(pred_json_list=pred_json_list)
        .build_mask(
            dicom_seg_result_list=dicom_seg_result_list, pred_json_list=pred_json_list
        )
        .build()
    )
