...
from code_ai.pipeline.dicomseg.schema.aneurysm import (
    AneurysmMaskRequest,
    AneurysmMaskSeriesRequest,
    AneurysmMaskInstanceRequest,
)
from code_ai.pipeline.dicomseg.schema.aneurysm import (
    AneurysmAITeamRequest,
    SortedRequest,
)
from code_ai.pipeline.dicomseg.schema.aneurysm import (
    AneurysmAITeam2Request,
    AneurysmMask2Request,
)
from code_ai.pipeline.dicomseg.schema.aneurysm import (
    AneurysmMaskSeries2Request,
    AneurysmMaskModel2Request,
)


from code_ai.pipeline.dicomseg.base import (
    PlatformJSONBuilder,
    ReviewBasePlatformJSONBuilder,
)
...
class AneurysmPlatformJSONBuilder(PlatformJSONBuilder[AneurysmAITeamRequest]):
    ...
    def __init__(self, **kwargs):
        ...
        self._series_dict: Optional[
            Dict[Union[SeriesTypeEnum, str], List[Union[FileDataset, DicomDir]]]
        ] = {}

    def reset(self):
        super().reset()
        self._series_dict: Optional[
            Dict[SeriesTypeEnum, List[Union[FileDataset, DicomDir]]]
        ] = {}

    def set_series_type(
        self,
        series_type: Union[SeriesTypeEnum, str],
    ...
    def build_mask_instance(
        ...
        **kwargs,
        ...
        dicom_sop_instance_uid = (
            source_images[main_seg_slice].get((0x008, 0x0018)).value
        )
    ...
    def build_mask_series(
        ...
        **kwargs,
        ...
        for index, reslut in enumerate(reslut_list):
            filter_cmd_data = list(
                filter(
                    lambda x: x["mask_index"] == str(reslut["mask_index"]),
                    pred_json_list,
                )
            )
            if filter_cmd_data:
            ...
            mask_instance = self.build_mask_instance(
                source_images, dcm_seg, *args, **kwargs
            )
    ...
    def set_mask(
        ...
        mask_dict = {"study_instance_uid": study_instance_uid, "group_id": group_id}
        series_list = []
        for index, result in enumerate(result_list):
            mask_series = self.build_mask_series(
                source_images=self._series_dict[result["series_name"]],
    ...
    def set_study(
        ...
        study_dict = self.build_study_basic_info(
            source_images=source_images, group_id=group_id
        )
        study_series_list = []
        study_model_list = []
        for index, (series_name, series_source_images) in enumerate(
            ...
            study_series = self.build_study_series(
                source_images=series_source_images, series_type=series_name
            )
            study_series_list.extend(study_series)
            study_model = self.build_study_model(
                pred_json_list=pred_json_list[index]["pred_json"],
                series_type=series_name,
            )
            study_model_list.extend(study_model)

        study_dict.update({"series": study_series_list, "model": study_model_list})
        self._study_request = StudyRequest.model_validate(study_dict)

        return self

    def build_study_series(
        self, source_images: List[Union[FileDataset, DicomDir]], *args, **kwargs
    ) -> Union[Dict[str, Any], Any]:
        if self.series_type:
            series_type = self.series_type
        else:
        ...
        for index, dicom_ds in enumerate(source_images):
            ...
            if series_instance_uid_dict.get(series_instance_uid) is None:
                ...
                series_instance_uid_dict.update(
                    {
                        series_instance_uid: {
                            "series_type": series_type,
    ...
    def build_study_model(
        self, pred_json_list: List[Dict[str, Any]], *args, **kwargs
    ) -> Union[Dict[str, Any], Any]:
        if self.series_type:
            study_model = dict(
    ...
    def get_mask_instance(
        ...
        **kwargs,
        ...
        for index, result in enumerate(result_data_list):
            ...
            filter_cmd_data = list(
                filter(
                    lambda x: str(x["mask_index"]) == str(result["mask_index"]),
                    pred_json_data_list,
                )
            )
            if filter_cmd_data:
            ...
            mask_instance_list.append(
                self.MaskInstanceClass.model_validate(mask_instance_dict)
            )
    ...
    def build_mask(
        ...
        if all((self._series_dict is not None, self.group_id is not None)):
            for index, (series_name, series_source_images) in enumerate(
                ...
                if index == 0:
                    study_instance_uid = (
                        series_source_images[0].get((0x0020, 0x000D)).value
                    )
                    mask_dict.update(
                        {
        ...
        for index, (series_name, series_source_images) in enumerate(
            ...
            mask_model_series: AneurysmMaskModel2Request = self.get_mask_model(
...
def use_create_dicom_seg_file(
    ...
    if series_name == "MRA_BRAIN":
        new_nifti_array = utils.get_array_to_dcm_axcodes(
            path_nii.joinpath("Pred.nii.gz")
        )
    else:
        new_nifti_array = utils.get_array_to_dcm_axcodes(
            path_nii.joinpath(f"{series_name}_pred.nii.gz")
        )
...
#                        "pred_json": pred_json},
#                       ]
def get_excel_to_pred_json(excel_file_path: str, intput_list: List[Dict[str, Any]]):
    """
    ...
    for intput_dict in intput_list:
        ...
        pred_json_list.append(
            {"series_name": intput_dict["series_name"], "data": series_name_pred}
        )
        # break
        # break
...
def execute_dicomseg_platform_json(_id: int, root_path: str, group_id: int):
    ...
    pred_json_list = get_excel_to_pred_json(
        excel_file_path=str(
            path_nii.parent.joinpath("excel", "Aneurysm_Pred_list.xlsx")
        ),
        intput_list=pred_nii_path_list,
    )

    (
        mar_brain_sorted_dcms,
        mar_brain_image,
        mar_brain_first_dcm,
        mar_brain_source_images,
    ) = utils.load_and_sort_dicom_files(str(mar_brain_path_dcms))
    # # Create DICOM-SEG files for each unique region
    mar_brain_result_list = use_create_dicom_seg_file(
        path_nii,
        "MRA_BRAIN",
        path_dcmseg,
        mar_brain_image,
    ...
    #
    (
        mip_pitch_sorted_dcms,
        mip_pitch_image,
        mip_pitch_first_dcm,
        mip_pitch_source_images,
    ) = utils.load_and_sort_dicom_files(str(mip_pitch_path_dcms))
    ...
    mip_yaw_sorted_dcms, mip_yaw_image, mip_yaw_first_dcm, mip_yaw_source_images = (
        utils.load_and_sort_dicom_files(str(mip_yaw_path_dcms))
    )
    ...
    result_list = [
        {"series_name": "MRA_BRAIN", "data": mar_brain_result_list},
        {"series_name": "MIP_Pitch", "data": mip_pitch_result_list},
        {"series_name": "MIP_Yaw", "data": mip_yaw_result_list},
    ]

    aneurysm_platform_json_builder = ReviewAneurysmPlatformJSONBuilder()
    aneurysm_platform_json = (
        aneurysm_platform_json_builder.set_group_id(group_id=group_id)
        ...
        .build_mask(dicom_seg_result_list=result_list, pred_json_list=pred_json_list)
        .build_sorted()
        .build_study(
    ...
    platform_json_path = output_series_folder.joinpath(
        path_root.joinpath("aneurysm_platform_json.json")
    )
...
def main():
    ...
    parser.add_argument(
        "--ID",
        type=str,
        default="01901124_20250617_MR_21404020048",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "/mnt/e/pipeline/新增資料夾/01901124_20250617_MR_21404020048/Image_nii/Pred.nii.gz "
        ],
        help="用於輸入的檔案",
    )
    parser.add_argument(
