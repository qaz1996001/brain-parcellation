...
from code_ai.pipeline.dicomseg.schema.cmb_old import MaskRequest,MaskSeriesRequest,MaskInstanceRequest
from code_ai.pipeline.dicomseg.schema.cmb_old import StudyRequest, StudySeriesRequest
from code_ai.pipeline.dicomseg.schema.cmb_old import CMBAITeamRequest
from code_ai import load_dotenv
from code_ai.pipeline.dicomseg.base import GROUP_ID
...
class CMBPlatformJSONBuilder(PlatformJSONBuilder[CMBAITeamRequest]):
    ...
    def set_mask(self, source_images: List[Union[FileDataset, DicomDir]],
        ...
        self._mask_request = MaskRequest.model_validate(mask_dict)
        return self
    ...
    def set_study(self, source_images: List[Union[FileDataset, DicomDir]],
        ...
        study_dict.update({'series':study_series,
                           # 'model':study_model
                           'aneurysm_lession':"{}".format(len(pred_json_list)),
                           })
...
def main():
    ...
    platform_json_path = output_series_folder.joinpath(path_nii.name.replace('.nii.gz',
                                                                             'cmb_platform_json_build_old.json'))
