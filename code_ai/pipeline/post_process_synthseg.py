...
if __name__ == "__main__":
    ...
    if save_mode == InferenceEnum.CMB:
        cmb_file_list = list(
            map(lambda cmd_file: pathlib.Path(cmd_file), args.cmb_file_list)
        )
        original_cmb_file_list: List[pathlib.Path] = list(
            map(
                lambda x: x.parent.joinpath(
                    f"synthseg_{x.name.replace('resample', 'original')}"
                ),
                cmb_file_list,
            )
        )
        ...
        template_coregistration_file_name = swan_file.parent.joinpath(
            f"{synthseg_basename}_from_{template_basename}"
        )
        cmd_str = TemplateProcessor.flirt_cmd_base.format(
            t1_file, swan_file, template_coregistration_file_name
        )
