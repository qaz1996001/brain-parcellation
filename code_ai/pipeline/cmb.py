...
class CMBServiceTF:
    label_index_name_mapping_dict = {
        0: "Background",
        1: "CSF",
        301: "brainstem",
        102: "L. cerebellum",
        103: "L. basal ganglion",
        104: "L. thalamus",
        105: "L. internal capsule",
        106: "L. external capsule",
        107: "L. corpus callosum",
        108: "L. DPWM",
        109: "L. frontal",
        110: "L. parietal",
        111: "L. occipital",
        ...
        114: "L. cingulate",
        202: "R. cerebellum",
        203: "R. basal ganglion",
        204: "R. thalamus",
        205: "R. internal capsule",
        206: "R. external capsule",
        207: "R. corpus callosum",
        208: "R. DPWM",
        209: "R. frontal",
        210: "R. parietal",
        211: "R. occipital",
    ...
    MODEL1_PATH = os.path.join(
        MODEL_DIR,
        "2025_02_10_MP-input64-aug_rot_bc10-bz32-unet5_32-bce_dice-Adam1e3_cosine_ema",
    )
    MODEL2_PATH = os.path.join(
        MODEL_DIR,
        "2025_02_10_MP-norm1-input26ch2_gauss_loc-aug2-bz64-Res32x3FPN-D128x4D1-cw-Adam1E3_ema",
    )

    def __init__(self) -> None:
        import tensorflow as tf

        self.importance_kernel = self.get_importance_kernel(
            self.patch_size, self.blend_mode, self.sigma
        )
        self.importance_map = tf.tile(
            tf.reshape(self.importance_kernel, shape=[1, *self.patch_size, 1]),
    ...
    def cmb_classify(
        ...
        target_spacing = np.array(
            [spacing[0], spacing[1], min(spacing)]
        )  # isotropical z-axis only
        target_spacing = (
            target_spacing / 2 if target_spacing[0] > 0.6 else target_spacing
        )  # force upsampling?
        image_arr = self.resize_volume(
            image_arr, spacing, target_spacing, dtype="float32"
        )
        if temp_path is not None:
            ...
            seg_arr, _ = self.load_synthseg_seg(
                seg_path=str(temp_path), target_size=image_arr.shape
            )
            ...
            df_pred, pred_label = self.object_analysis(
                image_arr, pred_map, target_spacing, model2
            )
            self.save_nii_trio(
                nifti_file=swan_path,
    ...
    def load_synthseg_seg(
        cls, seg_path="synthseg33.nii.gz", target_size=None, verbose=False
    ):
        # Load synthseg result
        x = nib.load(seg_path)
        x = nib.as_closest_canonical(x)  # to RAS space
        spacing = list(x.header.get_zooms())
        seg_arr = x.get_fdata().astype("uint16")[
            ::-1, ::-1, :
        ]  # RAS to LPS orientation

        if target_size != None:
            seg_arr = cls.resize_volume(
                seg_arr, target_size=target_size, dtype="uint16"
            )
            spacing = None
    ...
    @classmethod
    def load_volume(
        cls, path_volume, im_only=False, squeeze=True, dtype=None, LPS_coor=True
    ):
        """
        Load volume file.
    ...
    @classmethod
    def resize_volume(
        cls, arr, spacing=None, target_spacing=None, target_size=None, dtype="float32"
    ):
        ...
        if (spacing is not None) and (target_spacing is not None):
            ...
            out_vol = ndi.zoom(
                arr, zoom=scale, order=order, prefilter=True, grid_mode=False
            )
    ...
    def sliding_window_inference(
        ...
        **kwargs,
        ...
        window_slices = cls.get_window_slices(
    ...
    def object_analysis(
        ...
        gaussian = (
            gaussian_kernel(roi_size=(26, 26, 26), sigma=1 / 8)
            .numpy()
            .astype("float16")
        )
        ...
        if (pred_label.max() > 0) and (FP_reduction_th > 0):
            ...
            for p in props:
                ...
                TP_conf.append(
                    model(x, training=False).numpy()[0][0]
                )  # <-- model2 prediction
            TP_conf = np.stack(TP_conf).astype("float32") / 2  # covert to [0.0:1.0]
            pred_type = [
                "CMB" if x * 2 > uncertain_th else "Uncertain" for x in TP_conf
            ]
        else:
        ...
        props = regionprops_table(
            pred_label, pred_map, properties=("label", "bbox", "intensity_mean")
        )
    ...
    @classmethod
    def save_nii_trio(
        cls, nifti_file, pred_label, output_path="Pred.nii.gz", verbose=False
    ):
        # name output folder
    ...
    def save_label_table(
        ...
        for props in regionprops(label_arr):
            ...
            df_tsv.loc[lb, "class_name"] = df_pred.loc[
                df_pred["Pred_label"] == lb, "Pred_type"
            ].values[0]
            label_index = get_location(lb)
            df_tsv.loc[lb, "type"] = f"C{label_index}"  # location
            df_tsv.loc[lb, "type_name"] = cls.label_index_name_mapping_dict.get(
                label_index, ""
            )
            # df_tsv.loc[lb, 'LPS_coordinates'] = ",".join([str(x) for x in props.bbox])
            df_tsv.loc[lb, "pred_diameter"] = df_pred.loc[
                df_pred["Pred_label"] == lb, "Pred_diameter"
            ].values[0]
            df_tsv.loc[lb, "CMB_prob"] = df_pred.loc[
                df_pred["Pred_label"] == lb, "CMB_prob"
            ].values[0]
        ...
        with open(output_json_path_str, "w") as f:
            df_tsv.to_json(path_or_buf=f, orient="records", index=False)
...
if __name__ == "__main__":
    ...
    cmb_pipeline.cmb_classify(
        swan_path_str=args.swan_path_str,
