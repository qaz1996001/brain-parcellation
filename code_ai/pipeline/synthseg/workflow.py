    ...
    if args.template:
        ...
        if args.template_name:
            template_file_list = list(
                filter(lambda x: args.template_name in x.name, template_file_list)
            )

        template_resample_file_list = list(
            map(
        ...
        template_synthseg_file_list = list(
            map(
                lambda x: x.parent.joinpath(replace_suffix(x.name, "_synthseg.nii.gz")),
        ...
        template_synthseg33_file_list = list(
            map(
                lambda x: x.parent.joinpath(
                    replace_suffix(x.name, "_synthseg33.nii.gz")
                ),
                template_resample_file_list,
            )
        )
    ...
    if args.template:
        if args.cmb:
            cmb_file_list = list(
                map(
                    lambda x: x.parent.joinpath(
                        replace_suffix(x.name, f"_{args.cmb_file}.nii.gz")
                    ),
                    template_resample_file_list,
                )
            )
        ...
        if args.dwi:
            dwi_file_list = list(
                map(
                    lambda x: x.parent.joinpath(
                        replace_suffix(x.name, f"_{args.dwi_file}.nii.gz")
                    ),
                    template_resample_file_list,
                )
            )
        ...
        if args.wmh:
            wmh_file_list = list(
                map(
                    lambda x: x.parent.joinpath(
                        replace_suffix(x.name, f"_{args.wmh_file}.nii.gz")
                    ),
                    template_resample_file_list,
                )
            )
        ...
        david_file_list = list(
            map(
                lambda x: x.parent.joinpath(replace_suffix(x.name, "_david.nii.gz")),
                template_resample_file_list,
            )
        )
        wm_file_list = list(
            map(
                lambda x: x.parent.joinpath(replace_suffix(x.name, "_wm.nii.gz")),
                template_resample_file_list,
            )
        )
        ...
        synthseg5_file_list = list(
            map(
                lambda x: x.parent.joinpath(
                    replace_suffix(x.name, "_synthseg5.nii.gz")
                ),
                resample_file_list,
            )
        )
        synthseg33_file_list = list(
            map(
                lambda x: x.parent.joinpath(
                    replace_suffix(x.name, "_synthseg33.nii.gz")
                ),
    ...
    for i in range(len(file_list)):
        try:
            if args.template:
                ...
                resample_one(
                    str(template_file_list[i]), str(template_resample_file_list[i])
                )
                synth_seg.run(
                    path_images=str(template_resample_file_list[i]),
                ...
                original_seg_file, argmin = resampleSynthSEG2original_z_index(
                    file_list[i], resample_file_list[i], synthseg33_file_list[i]
                )
                ...
                template_synthseg33_basename = template_synthseg33_file_list[
                    i
                ].name.replace(".nii.gz", "")
                synthseg33_basename = synthseg33_file_list[i].name.replace(
                    ".nii.gz", ""
                )
                ...
                template_coregistration_file_name = template_synthseg33_file_list[
                    i
                ].parent.joinpath(
                    f"{synthseg33_basename}_from_{template_synthseg33_basename}"
                )

                flirt_cmd_base_str = TemplateProcessor.flirt_cmd_base.format(
                    template_synthseg33_file_list[i],
                ...
                if args.cmb:
                    ...
                    cmb_coregistration_file_name = "{}_from_{}".format(
                        synthseg33_file_list[i].parent.joinpath(
                            synthseg33_basename.replace(
                                "synthseg33", f"{args.cmb_file}"
                            )
                        ),
                        cmb_basename,
                    )
                    flirt_cmd_apply_str = TemplateProcessor.flirt_cmd_apply.format(
                        cmb_file_list[i],
                        synthseg33_file_list[i],
                ...
                if args.dwi:
                    ...
                    dwi_coregistration_file_name = (
                        f"{dwi_basename}_{synthseg33_basename}"
                    )
                    flirt_dwi_str = (
                        rf'export FSLOUTPUTTYPE=NIFTI_GZ && flirt -in "{dwi_file_list[i]}" -ref "{synthseg33_file_list[i]}" '
                        rf'-out "{dwi_coregistration_file_name}" '
                        rf'-init "{template_coregistration_file_name}.mat" '
                        rf"-applyxfm -interp nearestneighbour"
                    )
                    print("dwi")
                    print(flirt_dwi_str)
                ...
                out_nib = nib.Nifti1Image(
                    synthseg_array_wm, synthseg_nii.affine, synthseg_nii.header
                )
