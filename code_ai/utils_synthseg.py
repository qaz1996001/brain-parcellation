...
class SynthSeg:
    LABELS_DIR = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "resource", "labels_classes_priors"
    )
    MODEL_DIR = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "resource", "models"
    )
    ...
    def prepare_output_files(self, path_images, out_seg, recompute):
        ...
        if basename[-4:] == ".txt":
            ...
            out_synthseg, recompute_synthseg, _ = text_helper(
                out_seg, "path_segmentations"
            )
            out_synthseg33, recompute_synthseg33, _ = text_helper(
                out_seg, "path_segmentations"
            )
        ...
        # path_images is a folder
            ...
            # use helper on all outputs
            out_synthseg, recompute_synthseg, _ = helper_dir(
                out_seg, "path_segmentations", "", "synthseg"
            )
            out_synthseg33, recompute_synthseg33, _ = helper_dir(
                out_seg, "path_segmentations", "", "synthseg33"
            )
        ...
        # path_images is an image
            ...
            # use helper on all outputs
            out_synthseg, recompute_synthseg, _ = helper_im(
                out_seg, "path_segmentations", "", "synthseg"
            )
            out_synthseg33, recompute_synthseg33, _ = helper_im(
                out_seg, "path_segmentations", "", "synthseg33"
            )

        recompute_list = [
            recompute | re_seg
            for (re_seg, re_seg33) in zip(recompute_synthseg, recompute_synthseg33)
        ]
    ...
    def build_conver_model(
        ...
        last_tensor1 = KL.Lambda(lambda x: tf.cast(tf.argmax(x, axis=-1), "int32"))(
            input_tensor
        )
        last_tensor1 = layers.ConvertLabels(
        ...
        last_tensor1 = KL.Lambda(
            lambda x: tf.one_hot(tf.cast(x, "int32"), depth=2, axis=-1)
        )(last_tensor1)
        last_tensor1 = KL.Lambda(lambda x: tf.cast(tf.concat(x, axis=-1), "float32"))(
            [input_image, last_tensor1]
        )
        net_convert = Model(
            inputs=[input_tensor, input_image], outputs=last_tensor1, name="net_convert"
        )
        return net_convert
    ...
    def build_model(
        ...
        # build UNet 2
        net_unet2 = self.build_unet_model(
        ...
        net_convert = self.build_conver_model(
        ...
        net_parcellation = Model(
            inputs=net_parcellation.inputs, outputs=last_tensor, name="unet_parc"
        )
    ...
    def postprocess(
        ...
        segmentation_processor = SegmentationProcessor(
            shape, pad_idx, crop_idx, aff, labels_segmentation, fast, topology_classes
        )
        parcellation_processor = ParcellationProcessor(
            shape, pad_idx, crop_idx, aff, labels_parcellation
        )

        context = PostProcessContext(
            segmentation_processor,
            parcellation_processor,
            return_seg,
            return_posteriors,
        )
    ...
    # @profile
    def preprocess(
        self, path_image, ct, target_res=1.0, n_levels=5, crop=None, min_pad=None
    ):
        ...
        if n_dims == 2 and 1 < n_channels < 4:
            ...
            print(
                "WARNING: detected more than 1 channel, only keeping the first channel."
            )
            im = im[..., 0]
        ...
        if self.args is None:
            ...
            if args["robust"]:
                args["path_model_segmentation"] = os.path.join(
                    self.MODEL_DIR, "synthseg_robust_2.0.h5"
                )
            else:
                args["path_model_segmentation"] = os.path.join(
                    self.MODEL_DIR, "synthseg_2.0.h5"
                )

            args["path_model_parcellation"] = os.path.join(
                self.MODEL_DIR, "synthseg_parc_2.0.h5"
            )
            args["labels_segmentation"] = os.path.join(
                self.LABELS_DIR, "synthseg_segmentation_labels_2.0.npy"
            )
            args["labels_denoiser"] = os.path.join(
                self.LABELS_DIR, "synthseg_denoiser_labels_2.0.npy"
            )
            args["labels_parcellation"] = os.path.join(
                self.LABELS_DIR, "synthseg_parcellation_labels.npy"
            )
            args["names_segmentation_labels"] = os.path.join(
                self.LABELS_DIR, "synthseg_segmentation_labels_2.0_david2.npy"
            )
            args["names_parcellation_labels"] = os.path.join(
                self.LABELS_DIR, "synthseg_parcellation_names.npy"
            )
            args["topology_classes"] = os.path.join(
                self.LABELS_DIR, "synthseg_topological_classes_2.0.npy"
            )
    ...
    def run(self, path_images, path_segmentations, path_segmentations33):
        ...
        if self.net_unet2 is None:
            net_unet2, net_convert, net_parcellation = self.build_model(
                path_model_segmentation=path_model_segmentation,
        ...
        image, aff, h, im_res, shape, pad_idx, crop_idx = self.preprocess(
            path_image=path_images, ct=ct, crop=cropping, min_pad=min_pad
        )
    ...
    def build_model_by_5class(self, path_model_segmentation, labels_denoiser):
        assert os.path.isfile(path_model_segmentation), (
            "The provided model path does not exist."
        )
        # build first UNet
    ...
    def run_segmentations5(self, path_images, path_segmentations5):
        ...
        if self.net_unet is None:
            net_unet = self.build_model_by_5class(
                path_model_segmentation=path_model_segmentation,
                labels_denoiser=labels_denoiser,
            )
...
class VolumeProcessor:
    ...
    def crop_volume(self, volume):
        return edit_volumes.crop_volume_with_idx(
            volume, self.pad_idx, n_dims=3, return_copy=False
        )

    def align_volume(self, volume, aff_ref):
        return edit_volumes.align_volume_to_ref(
            volume, aff=np.eye(4), aff_ref=aff_ref, n_dims=3, return_copy=False
        )
...
class SegmentationProcessor(VolumeProcessor, SegmentationStrategy):
    def __init__(
        self, shape, pad_idx, crop_idx, aff, labels_segmentation, fast, topology_classes
    ):
    ...
    def process(self, post_patch_seg, return_seg, return_posteriors):
        ...
        post_patch_seg_mask = np.broadcast_to(
            post_patch_seg_mask[..., None], broadcast_shape
        )
        ...
        if not self.fast and self.topology_classes is not None:
            ...
            for topology_class in np.unique(self.topology_classes)[1:]:
                tmp_topology_indices = np.where(
                    self.topology_classes == topology_class
                )[0]
                tmp_mask = np.any(
        ...
        if return_seg:
            seg_patch = np.unique(self.labels_segmentation)[
                post_patch_seg.argmax(-1).astype("int32")
            ].astype("int32")
        else:
            seg_patch = None
            ...
            if self.crop_idx is not None:
                ...
                posteriors = (
                    np.zeros(
                        shape=[
                            *self.shape,
                            np.unique(self.labels_segmentation).shape[0],
                        ]
                    )
...
class ParcellationProcessor(VolumeProcessor, ParcellationStrategy):
    ...
    def process(self, post_patch_parc, seg_patch, return_seg, return_posteriors):
        ...
        parc_patch = self.labels_parcellation[
...
class PostProcessContext:
    ...
    def execute(self, post_patch_seg, post_patch_parc):
        seg_patch, post_patch_seg = self.segmentation_strategy.process(
            post_patch_seg, self.return_seg, self.return_posteriors
        )
        seg_patch = self.parcellation_strategy.process(
            post_patch_parc, seg_patch, self.return_seg, self.return_posteriors
        )
        seg, posteriors = self.segmentation_strategy.paste_back(
            seg_patch, post_patch_seg, self.return_seg, self.return_posteriors
        )
