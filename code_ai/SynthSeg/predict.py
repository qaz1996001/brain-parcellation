...
def prepare_output_files(
    path_images, out_seg, out_posteriors, out_resampled, out_volumes, recompute
):
    # check inputs
    ...
    recompute_list = [
        recompute | re_seg | re_post | re_res | re_vol
        for (re_seg, re_post, re_res, re_vol) in zip(
            recompute_seg, recompute_post, recompute_resampled, recompute_volume
        )
    ]
...
def preprocess(
    path_image, n_levels, target_res, crop=None, min_pad=None, path_resample=None
):
    ...
    if n_channels == 1:
        ...
        for i in range(im.shape[-1]):
            im[..., i] = edit_volumes.rescale_volume(
                im[..., i],
                new_min=0.0,
                new_max=1.0,
                min_percentile=0.5,
    ...
    if (
        min_pad is not None
    ):  # in SynthSeg predict use crop flag and then if used do min_pad=crop else min_pad = 192
        min_pad = utils.reformat_to_list(min_pad, length=n_dims, dtype="int")
        min_pad = [
            utils.find_closest_number_divisible_by_m(s, 2**n_levels, "higher")
            for s in min_pad
...
def build_model(
    ...
    if gradients:
        ...
        last_tensor = KL.Lambda(
            lambda x: (x - K.min(x)) / (K.max(x) - K.min(x) + K.epsilon())
        )(last_tensor)
...
def postprocess(
    ...
    if topology_classes is None:
        post_patch = edit_volumes.crop_volume_with_idx(
            post_patch, pad_idx, n_dims=3, return_copy=False
        )
    ...
    if keep_biggest_component:
        ...
        post_patch_mask = np.stack(
            [post_patch_mask] * tmp_post_patch.shape[-1], axis=-1
        )
        tmp_post_patch = edit_volumes.mask_volume(
            tmp_post_patch, mask=post_patch_mask, return_copy=False
        )
    ...
    seg_patch = labels_segmentation[post_patch.argmax(-1).astype("int32")].astype(
        "int32"
    )
    ...
    if crop_idx is not None:
        ...
        if n_dims == 2:
            ...
            posteriors[crop_idx[0] : crop_idx[2], crop_idx[1] : crop_idx[3], :] = (
                post_patch
            )
        elif n_dims == 3:
            seg[
    ...
    seg = edit_volumes.align_volume_to_ref(
        seg, aff=np.eye(4), aff_ref=aff, n_dims=n_dims, return_copy=False
    )
    posteriors = edit_volumes.align_volume_to_ref(
        posteriors, np.eye(4), aff_ref=aff, n_dims=n_dims, return_copy=False
    )
