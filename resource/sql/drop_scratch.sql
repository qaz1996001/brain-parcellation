-- 清除現有的觸發器（如果存在）
DROP TRIGGER IF EXISTS trigger_update_update_time_on_dcop_event_bt ON dcop_event_bt;
DROP TRIGGER IF EXISTS trigger_update_update_time_on_dcop_event_bth ON dcop_event_bth;

-- 清除現有的函數（如果存在）
DROP FUNCTION IF EXISTS update_update_time_on_user_task();

-- 清除現有的類型（如果存在）
DROP TYPE IF EXISTS dcop_status CASCADE;

-- 清除現有的索引（如果存在）
DROP INDEX IF EXISTS idx_event_bt_study_uid;
DROP INDEX IF EXISTS idx_event_bt_series_uid;
DROP INDEX IF EXISTS idx_event_bt_create_time;
DROP INDEX IF EXISTS idx_event_bt_ope_no;
DROP INDEX IF EXISTS idx_conf_bt_tool_id;
DROP INDEX IF EXISTS idx_conf_bt_status_code;
