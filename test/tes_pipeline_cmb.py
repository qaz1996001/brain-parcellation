...
def test_pipeline_synthseg5class_tensorflow():
    ...
    task_params = TaskInferenceParams(
        input_study_nifti_path=Path("/mnt/d/wsl_ubuntu/pipeline/sean/example_input"),
        output_study_nifti_path=Path("/mnt/d/wsl_ubuntu/pipeline/sean/example_output"),
    )
    ...
    for study_path in study_list:
        file_path_list = sorted(study_path.glob("T1FLAIR_AXI.nii.gz")) + sorted(
            study_path.glob("SWAN.nii.gz")
        )
        print(file_path_list)
        file_path_list = list(map(lambda x: str(x), file_path_list))
        func_params = {
            "Output_folder": str(output_study_nifti_path),
            "ID": study_path.name,
