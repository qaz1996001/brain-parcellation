...
from pyorthanc import AsyncOrthanc, Orthanc, query_orthanc, Study
    ...
    try:
        ...
        if isinstance(result, bytes):
            ...
            with zipfile.ZipFile(io.BytesIO(result)) as zip_ref:
                ...
                print(
                    f"Extracting {total_files} files from series {id_} to {output_path}"
                )
                ...
                for file_index, file_name in enumerate(file_list):
                    ...
                    output_file_path = os.path.join(
                        output_path, os.path.basename(file_name)
                    )
                    ...
                    if (file_index + 1) % 50 == 0 or file_index == total_files - 1:
                        print(
                            f"Progress: {file_index + 1}/{total_files} files processed"
                        )
                ...
                print(
                    f"Successfully extracted {total_files} DICOM files from series {id_} to {output_path}"
                )
            return True
        else:
...
async def main():
    ...
    query = {}
    print("main")
    results: List[Union[Study, Resource]] = query_orthanc(
        client=client, level=level, query=query
    )
