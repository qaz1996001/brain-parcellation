...
def test_pipeline_flirt():
    ...
    for study_path in study_list:
        ...
        template_file = study_path.joinpath(
            "synthseg_T1BRAVO_AXI_original_synthseg5.nii.gz"
        )
        template_basename: str = replace_suffix(template_file.name, "")
        input_basename: str = replace_suffix(input_file.name, "")
        template_coregistration_file = input_file.parent.joinpath(
            f"{input_basename}_from_{template_basename}.nii.gz"
        )
