...
if __name__ == "__main__":
    ...
    df = pd.read_csv(
        "/data/10TB/sean/monai_3D_latent/SPADEAutoencoderKL_more_dataset/code/dataset_t1_t2.csv"
    )
    print(df.columns)
    data_list = []
    ...
    for data in data_list[1:]:
        Inputs = pathlib.Path(
            data.replace("/data/a_dataset", "/data/10TB/sean/a_dataset")
        )
        ID = Inputs.parent.name
        Output_folder = Inputs.parent.parent

        func_params = {
            "cmd_str": "export PYTHONPATH={PYTHONPATH} && {PYTHON3}"
            " code_ai/pipeline/pipeline_synthseg_tensorflow.py"
            " --ID {ID} --Inputs {Inputs}"
            " --Output_folder {Output_folder}"
            " --InputsDicomDir {InputsDicomDir}".format(
