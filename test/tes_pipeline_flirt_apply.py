...
def test_pipeline_flirt():
    ...
    task_params = TaskInferenceParams(
        input_study_nifti_path=Path("/data/10TB/sean/Suntory/rename_nifti"),
        output_study_nifti_path=Path("/data/10TB/sean/Suntory/rename_nifti"),
    )
    ...
    for study_path in study_list:
        ...
        coregistration_mat = study_path.joinpath(
            "synthseg_ASLPROD_original_synthseg5_from_synthseg_T1BRAVO_AXI_original_synthseg5.mat"
        )
        # synthseg_ASLPROD_original_CMB_from_synthseg_T1BRAVO_AXI_original_CMB.nii.gz
        output_coregistration_path = study_path.joinpath(
            "synthseg_ASLPROD_original_CMB_from_synthseg_T1BRAVO_AXI_original_CMB.nii.gz"
        )
